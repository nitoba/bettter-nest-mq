...
Executable timeoutMs must be positive. A zero timeout is rejected before store acquisition. Named/versioned custom retry providers now resolve through static Nest DI and execute through the native typed-failure decision hook; producer-only contexts persist references without requiring those providers. See execution-extensions.md for schemas, predicates, budgets and rolling-version fencing.
...
The current pipeline is an MQ invocation, not the HTTP transport. Method/class UseGuards, UsePipes, UseInterceptors and UseFilters metadata is rejected instead of silently ignored. Application-wide HTTP enhancers do not apply to MQ handlers. The explicit UseMqGuards/Pipes/Interceptors/Filters pipeline has its own MQ context, shares the real attempt DI subtree, validates transformed values and drains single-use continuations. See execution-extensions.md; it is not implicit HTTP compatibility.
...
Worker concurrency limits the local supervisor. Process concurrency limits that particular handler within the worker. These are not cluster-wide controls. Global/per-key concurrency and rate-limit declarations are implemented through storage-backed QueueControls; see controls.md for explicit policy deployment and validation. Runtime custom retry providers are described in execution-extensions.md. Local limits are never presented as distributed coordination.
...
The repository remains version 0.0.0 and is not published to npm. Durable flows, persistent schedules and native transactional outbox are implemented. ORM transaction bridges, other storage wrappers and durable event subscriptions remain pending. Distributed QueueControls are available as documented in controls.md. Native PostgreSQL outbox transactions and managed publication are available as documented in outbox.md; publisher and worker enablement are independent.
