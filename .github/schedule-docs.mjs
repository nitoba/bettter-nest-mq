...
update('README.md', '## Persistent schedules', (input) => {
  let text = replaceOnce(
    input,
    'Core execution, distributed controls and the native PostgreSQL transactional outbox are implemented.',
    'Core execution, distributed controls, native PostgreSQL transactional outbox and persistent schedules are implemented.'
  )
  text = text.replaceAll(
    'Flows, schedules, ORM transaction bridges',
    'Flows, ORM transaction bridges'
  )
  text = text.replaceAll('flows, schedules and ORM outbox bridges', 'flows and ORM outbox bridges')
  text = replaceOnce(
    text,
    'See [transactional outbox]',
    'See [persistent schedules](docs/schedules.md), [transactional outbox]'
  )
  return replaceOnce(
    text,
...
update('docs/architecture.md', '## Implemented persistent scheduler', (input) => {
  let text = input.replaceAll(
    'flows, schedules and ORM transaction bridges',
    'flows and ORM transaction bridges'
  )
...
update('CHANGELOG.md', '### Persistent schedules', (input) => {
  let text = input.replace(
    'Still pending: flows, schedules, transactional outbox,',
    'Still pending: flows, external ORM transaction bridges,'
  )
  return replaceOnce(
    text,
    '## Unreleased',
