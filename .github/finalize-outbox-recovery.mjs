...
patch('tests/package/consumer/outbox-retry.ts', [
  ["const Value = z.union([z.string(), z.null(), z.object({ text: z.string() })])", `// Test-only switch simulates a destination schema changed after a publication failed.
...
const Value = z.union([z.string(), z.null(), z.object({ text: z.string() })]).refine(
  ...
  ['      assert.equal(jobsBefore, 0)', "      assert.equal(jobsBefore, 1, 'The uncertain-publication case already exists at its target')"],
  ["      const corrupted = await record(service, 'corrupt')", `      const existing = await record(service, 'already-enqueued')
  ...
  ["      assert.equal((await record(service, 'corrupt')).state, 'failed')", `      // The native reader correctly rejects a digest that no longer matches the
  ...
  ['      for (const [index, item] of accepted.entries()) {', '      for (const item of accepted) {'],
  ["        assert.deepEqual(result, index < inputs.length ? inputs[index] : 'control')", '        assert.deepEqual(result, item.request.payload)'],
  ['PASS post-restart publication and processing preserve job IDs/JSON/dispatch keys without rerunning business writes', 'PASS post-restart recovery preserves IDs/JSON/dispatch keys, reuses an existing target job and never reruns business writes']
])
patch('README.md', [
  ['event-assisted result waits and managed Kysely outbox queries are implemented.', 'event-assisted result waits, managed Kysely outbox queries and guarded failed-publication recovery are implemented.'],
  ['See [Kysely outbox transactions]', 'See [failed outbox recovery](docs/outbox-recovery.md), [Kysely outbox transactions]'],
  ['## Kysely outbox transactions', `## Recover failed outbox publications
...
patch('docs/roadmap.md', [
  ['event-assisted result waits and managed Kysely outbox queries** are implemented.', 'event-assisted result waits, managed Kysely outbox queries and guarded failed-outbox recovery** are implemented.'],
  ['docs/event-waits.md and docs/kysely-outbox.md.', 'docs/event-waits.md, docs/kysely-outbox.md and docs/outbox-recovery.md.'],
  [`## M6c — External ORM enrollment and administration — pending

Enrollment of caller-owned Kysely transactions, TypeORM/Prisma bridges, other adapters, failed-record reset/retry and richer recovery administration remain separate work. Preserve verified transaction identity, uncertain-commit semantics and borrowed ownership. A managed Kysely callback does not prove ownership of an arbitrary external transaction. Application outbox remains distinct from flow coordination.`, `## M6c — Guarded failed-publication recovery — implemented
...
Enrollment of caller-owned Kysely transactions, TypeORM/Prisma bridges, other adapters, audit history, pruning and bulk administration remain separate work. Preserve verified transaction identity, uncertain-commit semantics and borrowed ownership. A managed Kysely callback does not prove ownership of an arbitrary external transaction. Application outbox remains distinct from flow coordination.`],
  ['M6a native outbox + M6b managed Kysely implemented; external ORM enrollment pending', 'M6a native outbox + M6b Kysely + M6c recovery implemented; external ORM enrollment pending']
])
patch('docs/outbox.md', [
  ['A failed record is visible through diagnostics; this first slice does not export an automatic reset/retry-failed administration method.', 'Failed records can be inspected and recovered explicitly with MqOutboxService.retryFailed; see outbox-recovery.md for version guards, preserved attempts and required destination validation. Recovery is never automatic.'],
  ['configuration, preparation, availability, read, append, conflict and transaction misuse', 'configuration, preparation, availability, read, append, conflict, retry and transaction misuse'],
  ['The native PostgreSQL transaction boundary is the first outbox implementation. ORM transaction bridges, additional drivers, richer administrative recovery controls, flows and persistent schedules remain separate work.', 'Native PostgreSQL outbox, managed Kysely queries, guarded failed-publication retry, flows and persistent schedules are implemented. Caller-owned ORM transaction enrollment, additional drivers, audit history and bulk administration remain separate work.'],
  ['This does not add flows or external ORM transactions.', 'Schedules remain distinct from the separately implemented flows and do not enroll external ORM transactions.']
])
