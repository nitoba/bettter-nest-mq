...
patch('src/integrations/postgres.ts', 'postgresScheduleLayer', [
  ["import { postgresOutboxLayer }", "import { postgresScheduleLayer } from './postgres-schedule-resource.ts'\nimport { postgresOutboxLayer }"],
  ['  readonly outbox?: boolean', '  readonly outbox?: boolean\n  readonly schedules?: boolean'],
  ['  enabled: boolean\n): AcquiredConnection', '  enabled: boolean,\n  schedules: boolean\n): AcquiredConnection'],
  ['  if (!enabled) return resource\n  return { ...resource, outbox: (name) => postgresOutboxLayer(name, pool, schema, namespace) }', `  let result = resource
  ...
  return result`],
  ['  const outbox = options.outbox ?? false', '  const outbox = options.outbox ?? false\n  const schedules = options.schedules ?? false\n  if (schedules !== true && schedules !== false) throw new MqConnectionException(\'<postgres>\', \'configuration\')'],
  ['          outbox\n        )', '          outbox,\n          schedules\n        )'],
  ['          outbox\n        )', '          outbox,\n          schedules\n        )']
])
patch('src/module/mq-module.options.ts', 'MqScheduleOptions', [
  ["import type { MqConnectionMap }", "import type { MqScheduleOptions } from '../schedules/types.ts'\nimport type { MqConnectionMap }"],
  ['export interface MqExecutionOptions {', 'export interface MqExecutionOptions {\n  readonly scheduler?: boolean'],
  ['export interface MqModuleOptions {', 'export interface MqModuleOptions {\n  readonly schedules?: MqScheduleOptions'],
  ['export interface MqResolvedOptions {', 'export interface MqResolvedOptions {\n  readonly schedules: Readonly<Required<MqScheduleOptions>>']
])
patch('src/module/mq.configuration.ts', 'resolveScheduleOptions', [
  ["import { resolveJobPolicy }", "import { resolveScheduleOptions } from '../schedules/decorator.ts'\nimport { resolveJobPolicy }"],
  ...
  ['        workers: options.execution?.workers ?? true,', '        workers: options.execution?.workers ?? true,\n        scheduler: options.execution?.scheduler ?? true,']
])
patch('src/engine/mq-engine.host.ts', 'SchedulesCoordinator', [
  ["import { EngineSession }", "import { SchedulesCoordinator } from './schedules.ts'\nimport { EngineSession }"],
  ...
  ['    this.outbox = new OutboxCoordinator', '    this.schedules = new SchedulesCoordinator(this.session, registry, moduleRef, configuration.options.schedules)\n    this.outbox = new OutboxCoordinator'],
  ['    try {\n      await this.session.start(this.registry.queues(), plans)', '    const schedules = await this.schedules.prepare()\n    try {\n      await this.session.start(this.registry.queues(), plans, schedules)'],
  ['        await this.controls.initialize()', '        await this.controls.initialize()\n        await this.schedules.initialize()'],
  ['        await this.session.activateOutboxPublisher()', '        await this.session.activateOutboxPublisher()\n        await this.session.activateScheduler()']
])
