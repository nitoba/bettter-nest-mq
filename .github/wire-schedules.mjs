...
import { Effect, Layer, Runtime }`
  ...
  replace(
    ...
    'NamedStore | OperationStore | EngineWorker | Clock | NamedOutbox | EngineOutboxPublisher | NamedSchedule | OperationSchedule | EngineScheduler'
  )
  replace(
  ...
  replace(
    ...
    'start(queues: ReadonlyArray<QueueDefinition>, plans: readonly WorkerPlan[] = [], schedules: readonly CompiledSchedule[] = []): Promise<void>'
  )
  replace(
    '    this.plans = plans',
  ...
  replace(
    ...
    'Layer.merge(ClockLive, stores, operations, workers, outboxes, publisher, scheduleStores, scheduleAliases, scheduler)'
  )
  replace(
  ...
  replace(
    anchor,
    `  scheduleSources(): readonly string[] { return Object.freeze([...this.readySchedules.keys()]) }
...
if (!text.includes('JSON.parse(\'{"text":1}\')'))
  writeFileSync(test, text.replace('payload: { text: 1 }', 'payload: JSON.parse(\'{"text":1}\')'))
