...
function replace(path, before, after) {
  ...
  assert.equal(
    ...
    1,
    `Expected one documentation anchor in ${path}: ${before}`
  )
...
replace(
  ...
  'Status: Core execution, distributed controls and the native PostgreSQL transactional outbox are implemented.'
)
replace(
  'README.md',
  'Flows, schedules, transactional outbox and additional integrations remain on the roadmap.',
  'Flows, schedules, ORM transaction bridges and additional integrations remain on the roadmap.'
)
replace(
  'README.md',
  'See [PostgreSQL JSON fidelity]',
  'See [transactional outbox](docs/outbox.md), [PostgreSQL JSON fidelity]'
)
replace(
  'README.md',
  'and the internal outbox dependency does not enable the pending Nest transactional-outbox API.',
  'and outbox storage is enabled explicitly with postgres({ outbox: true }), not merely by installing its internal dependency.'
)
replace(
  'README.md',
  'flows, schedules and transactional outbox are still pending.',
  'flows, schedules and ORM outbox bridges are still pending. Native PostgreSQL outbox transactions and a managed publisher are available.'
)
replace(
  'README.md',
...
replace(
  'docs/dependencies.md',
  'even during PostgreSQL worker and distributed-control tests.',
  'even during PostgreSQL worker, distributed-control and transactional-outbox tests.'
)
replace(
  'docs/architecture.md',
  'flows, schedules and transactional outbox are still planned.',
  'flows, schedules and ORM transaction bridges are still planned. Native PostgreSQL outbox transactions and a managed publisher are implemented.'
)
replace(
  'docs/architecture.md',
  'Read contracts.md, connections.md and execution.md',
  'Read outbox.md, contracts.md, connections.md and execution.md'
)
replace(
  'docs/architecture.md',
  'The current internal outbox dependency does not implement a Nest outbox facade.',
  'The native PostgreSQL facade now implements managed transactions using one actual PoolClient, separate native/adapter JSON views, tracked-operation draining and poison-on-failure. Its opt-in source stores and upstream publisher share the existing runtime and pool. Publisher enablement is independent of worker enablement. Duplicate validation includes destination and dispatch key; repeated callbacks remain an application idempotency concern. No arbitrary external transaction/ORM bridge or automatic callback replay is promised.'
)
replace(
  'docs/architecture.md',
  'Factory/value workers are not currently supported.',
  'Factory/value workers are not currently supported. The pinned supervisor also requires queue/name/version uniqueness within one Worker even across different connections; use separate Worker Services for those identities. This limitation is validated before acquisition rather than leaking an engine startup failure.'
)
replace(
  'docs/architecture.md',
  'further distributed/crash/flow/outbox coverage',
  'further distributed/crash/flow/ORM transaction coverage'
)
replace(
  'docs/roadmap.md',
  'and **M3.1a distributed queue controls** are implemented.',
  ', **M3.1a distributed queue controls** and the **M6a native PostgreSQL outbox** are implemented.'
)
replace('docs/roadmap.md', 'and docs/controls.md.', ', docs/controls.md and docs/outbox.md.')
replace(
  'docs/roadmap.md',
  'this correction does not implement the pending flow/schedule/outbox features below.',
  'the subsequent native outbox integration reuses this corrected JSON boundary; flows and schedules remain pending.'
)
replace(
  'docs/roadmap.md',
  'the presence of the internal upstream outbox dependency does not provide a Nest outbox facade.',
  'native PostgreSQL outbox resources now share the same connection pool; corresponding resources for other adapters remain pending.'
)
replace(
  'docs/roadmap.md',
  '## M6 — Transactional outbox — pending\n\nBuild explicit transaction/append/publish APIs around prepared requests. Domain writes and outbox appends must share the actual transaction resource. Implement/test adapter-specific contexts and ORM bridges separately. Preserve rollback, publication only after commit, replay after enqueue-before-ack, conflicting IDs, independent publication/job retries and ownership. Application outbox stays separate from flow coordination.',
...
replace(
  'docs/roadmap.md',
  'M6 outbox after transaction bridges exist',
