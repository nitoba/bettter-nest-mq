...
function replace(path, before, after) {
  ...
  assert.equal(source.split(before).length - 1, 1, `Documentation anchor not found exactly once: ${path}: ${before}`)
  writeFileSync(path, source.replace(before, after))
}

replace('README.md', 'Status: M3 core execution is implemented.', 'Status: M3 core execution and M3.1a distributed controls are implemented.')
replace('README.md', 'See [execution](docs/execution.md)', 'See [distributed controls](docs/controls.md), [execution](docs/execution.md)')
replace('README.md', 'Named custom retry providers, distributed-control decorators, durable events', 'Named custom retry providers, durable events')
replace('README.md', '## Declare a queue and a worker', `## Distributed controls
...
replace('docs/execution.md', 'Global/per-key concurrency, rate-limit decorators and runtime policy providers remain the next extension; no local semaphore is advertised as distributed coordination.', 'Global/per-key concurrency and rate-limit declarations are implemented through storage-backed QueueControls; see controls.md for explicit policy deployment and validation. Runtime custom retry providers remain a separate extension. Local limits are never presented as distributed coordination.')
replace('docs/execution.md', 'other storage wrappers, distributed control APIs and durable event subscriptions remain separate milestones.', 'other storage wrappers and durable event subscriptions remain separate milestones. Distributed QueueControls are available as documented in controls.md.')
replace('docs/connections.md', 'distributed-control APIs remain a planned extension.', 'QueueControls now exposes global/per-key concurrency and rate-limit declarations through the controlled-store protocol; see controls.md for qualification and deployment rules.')
...
replace('docs/architecture.md', 'Distributed controls must use storage-backed coordination and multiple-replica tests, not local semaphores.', 'Distributed controls are implemented through QueueControls metadata and an internal protocol-dispatch view over the existing raw store. Global/per-key concurrency and fixed-window admission call the upstream controlled operations; no local semaphore or new lease algorithm is substituted. The raw persistence token stays stable. Replica startup validates persisted policy read-only; explicit coordinated deployment reconciles without disabling omissions. See controls.md for group checks, partial multi-store deployments, typed dispatch keys and separate-process PostgreSQL qualification.')
replace('AGENTS.md', 'Read README.md and docs/contracts.md,', 'Read docs/controls.md, README.md and docs/contracts.md,')
replace('AGENTS.md', 'M3 core producer/worker execution are implemented.', 'M3 core producer/worker execution plus M3.1a distributed controls are implemented.')
replace('AGENTS.md', '- Local concurrency is not distributed concurrency. Custom retry providers, distributed-control decorators and event-based waits must not be advertised before implementation.', `- Local concurrency is not distributed concurrency. QueueControls uses the upstream controlled-store claim/settlement/release/recovery protocol; never substitute process-local limits or plain claims for an enabled policy.
...
replace('CONTRIBUTING.md', 'Future flow, schedule, outbox and distributed-control APIs must be real integrations, never simulated methods.', 'Distributed QueueControls are implemented and documented in docs/controls.md. Future flow, schedule and outbox APIs must be real integrations, never simulated methods. Keep normal policy validation read-only and qualify shared limits with actual independent PostgreSQL worker processes.')
...
`)
replace('CHANGELOG.md', 'additional drivers, distributed-control APIs, named custom retry providers', 'additional drivers, named custom retry providers')
