...
export function compileWorker(options: WorkerOptions, invocations: ReadonlyArray<WorkerInvocation>, shutdown: MqShutdownOptions): WorkerPlan {
  const tag: `nestjs.worker/${string}` = `nestjs.worker/${options.name}`
  const token = Worker.service(tag)
  const handlers = invocations.map((invocation) => Worker.handle(invocation.compiled, (payload) => Effect.gen(async function* () {
    const context = yield* JobContext
    const signal = yield* CurrentAbortSignal
    const jobContext: JobExecutionContext = Object.freeze({
      jobId: context.jobId, queue: context.queue, name: context.name, version: context.version,
      connection: invocation.registered.identity.connection, attempt: context.attempt,
      attemptsMax: context.attemptsMax, delivery: context.delivery, workerId: context.workerId,
      metadata: Object.freeze({ ...context.metadata }), signal
    })
    try { return Result.ok(await invocation.invoke(payload, jobContext)) }
    catch (cause) {
      if (cause instanceof JobFailureException) return Result.err(await validateDomainFailure(invocation.registered, cause))
      throw cause
    }
  }), invocation.options))
  const { name: _name, ...settings } = options
  return { name: options.name, token, layer: token.layer(() => ({ ...settings, handlers, shutdown })) }
}
