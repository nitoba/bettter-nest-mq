...
import { withFreshControlledClock } from './controlled-clock.ts'
...
export function controlledStore(store: JobStoreContract): ControlledJobStoreContract | undefined {
  if (
    !(
      'getControls' in store && store.getControls instanceof Function &&
      ...
      'cancelControlled' in store && store.cancelControlled instanceof Function
    )
  ) return undefined
...
function normalize<Value>(result: Result<Value, JobStoreError>, operation: string): Effect<Value, JobStoreFailure> {
  if (Result.isOk(result)) return success(result.value)
  if (result.error instanceof JobStoreFailure) return failure(result.error)
  const error = new JobStoreFailure({ operation, retryable: result.error instanceof ControlsRevisionMismatchError, message: `Controlled ${operation} failed` })
...
export function dispatchStore(store: JobStoreContract, queues: readonly QueueDefinition[]): JobStoreContract {
  ...
  async function policy(queue: QueueName, failClosed = false): Promise<Effect<QueueControlsRecord | undefined, JobStoreFailure>> {
    const result = normalize(await extension.getControls({ queue }), 'getControls')
    if (Result.isError(result)) return failure(result.error)
    if (failClosed && required.has(queue) && !result.value?.enabled) return failure(new JobStoreFailure({ operation: 'claim', retryable: false, message: 'A declared queue control policy is missing or disabled' }))
    return success(result.value?.enabled ? result.value : undefined)
  }

  async function jobPolicy(jobId: Parameters<JobStoreContract['getJob']>[0]['jobId']): Promise<Effect<QueueControlsRecord | undefined, JobStoreFailure>> {
  ...
  const forwarded: JobStoreContract = {
    ...
    retry: store.retry.bind(store), requestCancellation: store.requestCancellation.bind(store),
    promote: store.promote.bind(store), remove: store.remove.bind(store), pause: store.pause.bind(store), resume: store.resume.bind(store), pausedQueues: store.pausedQueues.bind(store),
    async claim(request) {
      ...
      return success({ jobs: claimed.value.jobs, wakeToken: claimed.value.wakeToken, nextRunAt: claimed.value.nextEligibleAtMs ?? claimed.value.nextRunAtMs })
    },
    async settle(request) {
      const controls = await jobPolicy(request.jobId)
      if (Result.isError(controls)) return failure(controls.error)
      if (controls.value === undefined) return store.settle(request)
      const revision = controls.value.revision
      const settled = await withFreshControlledClock(store, request.jobId, request.now, (now) => {
        const outcome = request.outcome.type === 'retry'
          ? { ...request.outcome, runAt: request.outcome.runAt + (now - request.now) }
          : request.outcome
        return extension.settleControlled({ ...request, outcome, now, controlsRevision: revision })
      })
      if (Result.isError(settled) && (settled.error instanceof LeaseLostError || settled.error instanceof SettlementConflictError)) return failure(settled.error)
    ...
    async release(request) {
      ...
      const revision = controls.value.revision
      const released = await withFreshControlledClock(store, request.jobId, request.now, (now) =>
        extension.releaseControlled({ ...request, now, controlsRevision: revision })
      )
      if (Result.isError(released) && released.error instanceof LeaseLostError) return failure(released.error)
    ...
    async cancel(request) {
      ...
      const revision = controls.value.revision
      const cancelled = await withFreshControlledClock(store, request.jobId, request.now, (now) =>
        extension.cancelControlled({ ...request, now, controlsRevision: revision })
      )
      if (Result.isError(cancelled) && cancelled.error instanceof JobNotCancellableError) return failure(cancelled.error)
    ...
    async recoverStalled(request) {
      ...
      for (const queue of queues) {
        ...
        if (Result.isError(name)) return failure(new JobStoreFailure({ operation: 'recoverStalled', retryable: false, message: 'Invalid registered queue name' }))
        const controls = await policy(name.value)
        if (Result.isError(controls)) return failure(controls.error)
        if (controls.value === undefined) { nativeSweepNeeded = true; continue }
        ...
        const controlled = { ...request, queue: name.value, controlsRevision: controls.value.revision }
        const recovered = normalize(await extension.recoverStalledControlled(limit === undefined ? controlled : { ...controlled, limit }), 'recoverStalled')
        if (Result.isError(recovered)) return failure(recovered.error)
        transitions.push(...recovered.value.transitions)
      }
      // Preserve native namespace recovery for ordinary queues. At this same timestamp,
      // controlled queues have already gone through their fenced recovery operation.
      if (nativeSweepNeeded) {
        const limit = request.limit === undefined ? undefined : request.limit - transitions.length
        if (limit !== 0) {
          const recovered = await store.recoverStalled(limit === undefined ? request : { ...request, limit })
          if (Result.isError(recovered)) return failure(recovered.error)
          transitions.push(...recovered.value.transitions)
        }
      }
