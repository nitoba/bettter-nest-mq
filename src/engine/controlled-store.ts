...
const success = <Value>(value: Value) => Effect.fromResult(Result.ok(value))
const failure = <Failure>(error: Failure) => Effect.fromResult(Result.err(error))
    ...
    async recoverStalled(request) {
      if (request.queue === undefined) {
        ...
        for (const queue of queues) {
          ...
          if (limit === 0) break
          const next = limit === undefined ? { ...request, queue: name.value } : { ...request, queue: name.value, limit }
