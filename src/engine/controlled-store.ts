...
export function dispatchStore(
  ...
  async function policy(
    queue: QueueName,
    failClosed = false
  ): Promise<Result<QueueControlsRecord | undefined, JobStoreFailure>> {
    ...
    async recoverStalled(request) {
      if (request.queue === undefined) {
        ...
        for (const queue of queues) {
          ...
          const next =
            limit === undefined ? { ...request, queue: named } : { ...request, queue: named, limit }
