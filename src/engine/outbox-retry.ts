...
export function retryOutboxRecord(
  record: OutboxRecord,
  options: OutboxRetryOptions,
  nowMs: number
): OutboxRecord {
  if (record.state !== 'failed')
    throw new MqOutboxException('retry', 'Only failed publications can be retried')
  const expected = options.expected
  if (
    record.updatedAtMs !== expected.updatedAtMs ||
    record.attemptsMade !== expected.attemptsMade ||
    record.attemptsMax !== expected.attemptsMax
  ) {
    throw new MqOutboxException('conflict', 'The outbox record changed after it was inspected')
  }
  if (record.request.id === undefined)
    throw new MqOutboxException('retry', 'Retry requires the original stable job id')
  if (!Number.isSafeInteger(nowMs) || nowMs < 0)
    throw new MqOutboxException('retry', 'Invalid retry timestamp')
  const updatedAtMs = Math.max(nowMs, record.updatedAtMs + 1)
  const attemptsMax = record.attemptsMade + options.attempts
  if (!Number.isSafeInteger(updatedAtMs) || !Number.isSafeInteger(attemptsMax))
    throw new MqOutboxException(
      'retry',
      'Retry would overflow the supported timestamp or attempt range'
    )
  ...
  if (Result.isError(next))
    throw new MqOutboxException('retry', 'The failed publication cannot be requeued safely', {
      cause: next.error
    })
  return next.value
}
...
export class OutboxRetryAdapter {
  ...
  retry(previous: OutboxRecord, next: OutboxRecord): Promise<void> {
    if (this.closed)
      return Promise.reject(new MqOutboxException('unavailable', 'Outbox recovery is closed'))
...
export function bindOutboxRetry(store: OutboxStore, write: OutboxRetryWrite): OutboxRetryAdapter {
  if (adapters.has(store))
    throw new MqOutboxException('configuration', 'Outbox retry capability already bound')
...
export function outboxRetryAdapter(store: OutboxStore): OutboxRetryAdapter {
  ...
  if (adapter === undefined)
    throw new MqOutboxException(
      'unavailable',
      'This outbox adapter does not support administrative retry'
    )
  return adapter
}
