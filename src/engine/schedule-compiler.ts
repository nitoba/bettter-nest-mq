...
export async function compileSchedule<Input>(registered: RegisteredJob, supplied: ScheduleOptions<Input>, config: MqScheduleOptions = {}): Promise<CompiledSchedule> {
  ...
  if (registered.controls?.perKeyConcurrency !== undefined || contract.getDispatchKey(payload) !== undefined) throw new MqScheduleException('definition', 'The pinned scheduler cannot persist dispatch keys; schedule a coordinator job for per-key work')
  if (registered.policy.delayMs !== 0) throw new MqScheduleException('definition', 'A scheduled job cannot inherit a relative enqueue delay; its cadence defines occurrence time')
  const retry = options.retry ?? registered.policy.retry
  const job = compileJob(registered)
  let common = {
    payload, metadata: options.metadata ?? {}, attempts: retry.attempts,
    backoff: compileBackoff(retry), priority: options.priority ?? registered.policy.priority,
    misfire: options.misfire ?? { strategy: 'run-once' as const }, overlap: options.overlap ?? 'allow', timeZone: options.timeZone ?? 'UTC'
  }
  const timeoutMs = options.timeoutMs ?? registered.policy.timeoutMs
  let cadence: JobScheduleOptions<CompiledJob> = options.cron === undefined ? { ...common, everyMs: options.everyMs } : { ...common, cron: options.cron }
  if (timeoutMs !== undefined) cadence = { ...cadence, timeoutMs }
  try {
    const draft = job.schedule(options.key, cadence)
    const registry = JobSchedules.define({ group, schedules: [draft] })
    const schedule = registry.schedules[0]
    if (schedule === undefined) throw new MqScheduleException('definition', 'Schedule registry contains no declaration')
    const verified = await JobSchedules.encodePayload(schedule)
    if (Result.isError(verified)) throw new MqScheduleException('definition', 'Schedule payload could not be encoded', { cause: verified.error })
...
export async function compileSchedules(jobs: readonly RegisteredJob[], options: MqScheduleOptions = {}): Promise<readonly CompiledSchedule[]> {
  ...
  for (const registered of jobs) {
    for (const definition of registered.schedules ?? []) {
      ...
      const key = JSON.stringify([registered.identity.connection, item.schedule.group, item.schedule.key])
      if (keys.has(key)) throw new MqScheduleException('definition', 'Schedule group/key must be unique within a connection')
      keys.add(key)
      compiled.push(item)
    }
  }
