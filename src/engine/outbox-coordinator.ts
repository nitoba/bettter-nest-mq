...
export class OutboxCoordinator implements OutboxMonitor {
  ...
  async retryFailed(source: string, id: string, options: OutboxRetryOptions): Promise<OutboxSnapshot> {
    const copied = copyOutboxRetryOptions(options)
    const parsed = makeOutboxId(id)
    if (Result.isError(parsed)) throw new MqOutboxException('retry', 'Invalid outbox id', { cause: parsed.error })
    return this.withSource(source, async (store, prepare) => {
      ...
      if (Result.isError(result)) throw new MqOutboxException('read', 'Unable to inspect the failed publication', { cause: result.error })
      const record = result.value
      if (record === undefined) throw new MqOutboxException('retry', 'The outbox record does not exist')
      // Check the optimistic guard and bounds before asynchronous schema validation.
      retryOutboxRecord(record, copied, Date.now())
      const validated = await prepare({ id: record.id, job: { connection: record.target, request: record.request }, attempts: record.attemptsMax })
      if (!isDeepStrictEqual(validated.request, record.request)) throw new MqOutboxException('retry', 'Retry cannot change the original prepared request')
