...
import type { JobDefaultsInput, JobDefinition as EngineJobDefinition, PersistedBackoff } from 'better-effect-mq'
...
export type CompiledJob = EngineJobDefinition<string, string, number, Codec<ContractValue>, Codec<ContractValue>, Codec<DomainFailure>, NamedStoreToken>
...
function schemaCodec(schema: ValueSchema): Codec<ContractValue> {
  return Codec.make<ContractValue>({
    encode: async (value) => {
      try { return Result.ok(JSON.parse(await encodeSchema(schema, value))) }
      catch { return Result.err(new JobEncodeFailure({ message: 'The value cannot be encoded by its declared job schema' })) }
    },
    decode: async (value) => {
      try { return Result.ok(await validateSchema(schema, value)) }
      catch { return Result.err(new JobDecodeFailure({ message: 'Persisted data does not satisfy its declared job schema' })) }
    }
  })
}
  ...
  return Codec.make<DomainFailure>({
    encode: async (failure) => {
      if (schema === undefined || !(failure instanceof JobFailureException)) {
        return Result.err(new JobEncodeFailure({ message: 'No valid domain-failure contract was declared' }))
      }
      try { return Result.ok(JSON.parse(await encodeSchema(schema, failure.failure))) }
      catch { return Result.err(new JobEncodeFailure({ message: 'The domain failure does not satisfy its declared schema' })) }
    },
    decode: async (value) => {
      if (schema === undefined) return Result.err(new JobDecodeFailure({ message: 'No domain-failure contract was declared' }))
      try { return Result.ok(new JobFailureException(await validateSchema(schema, value))) }
      catch { return Result.err(new JobDecodeFailure({ message: 'Persisted domain failure does not satisfy its schema' })) }
    }
  })
}
...
export function compileBackoff(options: RetryOptions): PersistedBackoff {
  ...
  if (backoff.type === 'custom') throw new ContractDefinitionException('Named custom retry providers are not supported by the execution bridge yet')
  let compiled: PersistedBackoff
  switch (backoff.type) {
    case 'fixed': compiled = { type: 'constant', delayMs: backoff.delayMs }; break
    case 'linear': compiled = { type: 'linear', delayMs: backoff.initialDelayMs, incrementMs: backoff.incrementMs }; break
    case 'exponential': compiled = { type: 'exponential', delayMs: backoff.initialDelayMs, factor: backoff.factor }; break
  }
...
export function compileJob(registered: RegisteredJob): CompiledJob {
  ...
  if (policy.timeoutMs === 0) throw new ContractDefinitionException('Executable job timeouts must be greater than zero')
...
export async function validateDomainFailure(job: RegisteredJob, failure: DomainFailure): Promise<DomainFailure> {
  const schema = job.contract.schemas.failure
  if (schema === undefined) throw new ContractDefinitionException('JobFailureException requires a declared failure schema')
