...
interface ReadyConnection { readonly store: JobStoreContract; readonly snapshot: MqConnectionSnapshot }
interface RunningWorker { readonly name: string; readonly handle: WorkerHandle }
...
export class EngineSession implements MqConnectionMonitor, WorkerMonitor {
  ...
  constructor(connections: MqConnectionMap | undefined, shutdown: MqShutdownOptions = {}) {
    ...
    this.shutdown = Object.freeze({ gracePeriodMs: shutdown.gracePeriodMs ?? 30_000, abortAfterGracePeriod: shutdown.abortAfterGracePeriod ?? true })
  }

  get state(): MqEngineState { return this.currentState }
  connections(): ReadonlyArray<MqConnectionSnapshot> { return this.snapshots }
  ...
  private async initialize(queues: ReadonlyArray<QueueDefinition>): Promise<void> {
    ...
    try {
      this.assertStarting()
      if (this.configured === undefined) { this.currentState = 'disabled'; return }
      ...
      this.assertStarting()
      if (bindings.length === 0 && this.plans.length === 0) { this.currentState = 'ready'; return }
      ...
      this.runtime = await Runtime.make(Layer.merge(ClockLive, stores, workers), {
        onCleanupFailure: (diagnostic) => { this.runtimeCleanupErrors.push(new Error('MQ runtime cleanup diagnostic', { cause: diagnostic })) }
      })
      ...
      for (const binding of bindings) {
        ...
        } catch (cause) { throw new MqConnectionException(binding.name, 'protocol', { cause }) }
        ...
        ready.set(binding.name, {
          ...
          snapshot: Object.freeze({
            name: binding.name, adapter: descriptor.adapter, adapterVersion: descriptor.adapterVersion,
            protocolVersion: descriptor.protocolVersion, layoutVersion: descriptor.layoutVersion,
            ownership: binding.connection.ownership, capabilities: Object.freeze({ ...descriptor.capabilities })
          })
        })
      }
      this.assertStarting()
      ...
      const primary = cause instanceof MqConnectionException || cause instanceof MqEngineStateException ? cause : new MqConnectionException(acquiring, 'acquire', { cause })
      try { await this.cleanup() }
      catch (cleanupCause) { throw new AggregateError([primary, cleanupCause], 'MQ startup failed and resource cleanup also failed', { cause: primary }) }
      finally { if (!this.stopRequested) this.currentState = 'failed' }
      throw primary
    }
  }
  ...
  private async activate(): Promise<void> {
    ...
    const runtime = this.runtime
    if (this.state !== 'ready' || runtime === undefined) throw new MqEngineStateException(this.state, 'start workers')
    for (const plan of this.plans) {
      this.assertStarting()
      const result = await runtime.run(() => Effect.gen(async function* () { return Result.ok(yield* plan.token) }))
      if (Result.isError(result)) throw new MqJobException('binding', 'Worker activation failed', { cause: result.error })
      this.runningWorkers.push({ name: plan.name, handle: result.value })
    }
    this.assertStarting()
  }

  async runOperation<Value, Failure>(operation: () => JobOperation<Value, Failure, NamedStoreToken, true>): Promise<Result<Value, Failure>> {
    const runtime = this.runtime
    if (this.state !== 'ready' || runtime === undefined) throw new MqJobException('binding', `The MQ engine is ${this.state}`)
    return runtime.run(() => Effect.gen(async function* () { return Result.ok(yield* operation()) }))
  }

  workers(): readonly WorkerSnapshot[] {
    return Object.freeze(this.runningWorkers.map(({ name, handle }) => Object.freeze({ name, id: handle.id, state: handle.state, activeCount: handle.activeCount })))
  }
  ...
  async awaitIdle(options: WorkerIdleOptions = {}): Promise<void> {
    ...
    await Promise.all(this.runningWorkers.map(async ({ handle }) => { await handle.awaitIdle(options) }))
  }
  ...
  async probe(name: string): Promise<MqConnectionSnapshot> {
    return this.withStore(name, async (store) => {
      try { const result = await store.counts(); if (Result.isError(result)) throw result.error }
      catch (cause) { throw new MqConnectionException(name, 'probe', { cause }) }
  ...
  private cleanup(): Promise<void> { this.cleaning ??= this.releaseResources(); return this.cleaning }

  private async releaseResources(): Promise<void> {
    const failures: Error[] = []
    try { await this.runtime?.dispose({ gracePeriod: this.shutdown.gracePeriodMs, abortAfterGracePeriod: this.shutdown.abortAfterGracePeriod }) }
    catch (cause) { failures.push(cleanupError(cause)) }
    finally {
