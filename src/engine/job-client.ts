import type {
  JobAttemptView, JobEnqueueOptions as EngineEnqueueOptions, JobRecordView, DecodedJobFailure
...
function enqueueOptions(registered: RegisteredJob, options: JobEnqueueOptions = {}): EngineEnqueueOptions {
  ...
  const fields = { priority: options.priority ?? policy.priority, attempts: retry.attempts, backoff: compileBackoff(retry) }
...
async function boundedWait(session: EngineSession, job: CompiledJob, id: string, options: JobWaitOptions = {}): Promise<ContractValue> {
  if (options.strategy !== undefined && options.strategy !== 'polling') throw new MqJobException('awaitResult', 'Only polling waits are supported by this integration')
...
async function cancelJob(session: EngineSession, registered: RegisteredJob, job: CompiledJob, id: string): Promise<void> {
  ...
  unwrap(await session.withStore(registered.identity.connection, (store) => store.requestCancellation({ jobId: record.id, now: Date.now() })), 'cancel')
}
...
export function createJobClient(session: EngineSession, registered: RegisteredJob, job: CompiledJob): JobClient<ContractValue, ContractValue> {
  return {
    ...
    async prepare(payload, options) {
      const configured = enqueueOptions(registered, options)
      const value = JSON.parse(payload)
      const request = unwrap(await session.runOperation(() => job.prepare(value, configured)), 'prepare')
    ...
    async cancel(id) { await cancelJob(session, registered, job, id) },
