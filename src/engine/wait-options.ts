...
export function validateWaitOptions(options: JobWaitOptions): void {
  if (options.strategy !== undefined && options.strategy !== 'polling' && options.strategy !== 'events') {
  ...
  for (const [name, value] of Object.entries({ timeoutMs: options.timeoutMs, pollIntervalMs: options.pollIntervalMs, pollFallbackMs: options.pollFallbackMs })) {
    if (value === undefined) continue
    requireInteger(value, `wait.${name}`, name === 'timeoutMs' ? 0 : 1)
    if (value > 2_147_483_647) throw new RangeError(`wait.${name} exceeds the supported timer range`)
  }
}
