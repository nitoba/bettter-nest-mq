...
export type ScheduleRegistry = JobSchedulesDefinition<
  string,
  ScheduleDrafts,
  readonly OperationStoreToken[]
>
...
export function scheduleToken(name: string): NamedScheduleToken {
  return JobScheduleStore.for(namedStoreToken(name))
}
export function scheduleAlias(name: string): Layer<OperationSchedule, NamedSchedule> {
  return Layer.gen(JobScheduleStore.for(operationStoreToken(name)), async function* () {
    return yield* scheduleToken(name)
  })
}
export function scheduleRegistries(
  names: readonly string[],
  compiled: readonly CompiledSchedule[],
  group: string
  ...
  for (const name of names) {
    const groups = new Set([
      group,
      ...compiled
        .filter((entry) => entry.registered.identity.connection === name)
        .map((entry) => entry.schedule.group)
    ])
    for (const current of groups)
      registries.push(
        JobSchedules.define({ group: current, schedules: [], stores: [operationStoreToken(name)] })
      )
  }
  return registries
}
export function declarationRegistry(entry: CompiledSchedule): ScheduleRegistry {
  return JobSchedules.define({
    group: entry.schedule.group,
    schedules: [entry.draft],
    stores: [entry.draft.job.store]
  })
}
export function schedulerLayer(
  registries: readonly ScheduleRegistry[],
  options: Required<MqScheduleOptions>,
  onError: () => void
) {
  ...
  return Scheduler.layer(() => ({
    ...
    maxStoreRetries: options.maxStoreRetries,
    retryDelayMs: options.retryDelayMs,
    startupReconcile: false,
    onError: () => {
      onError()
      logger.warn('A scheduler operation failed; inspect schedule state and connection health')
    }
  }))
}
