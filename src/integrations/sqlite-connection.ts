...
type Source<Database extends object> =
  | { readonly ownership: 'owned'; readonly path: string }
  | { readonly ownership: 'borrowed'; readonly database: Database }

function assertOptions<Options extends object>(options: Options, allowed: readonly string[]): void {
  const prototype = Object.getPrototypeOf(options)
  if (prototype !== Object.prototype && prototype !== null)
    throw new Error('SQLite options must be a plain object')
  for (const key of Reflect.ownKeys(options)) {
    if (key !== String(key) || !allowed.includes(String(key)))
      throw new Error(`Unsupported SQLite option ${String(key)}`)
    const descriptor = Object.getOwnPropertyDescriptor(options, key)
    if (descriptor?.get !== undefined || descriptor?.set !== undefined)
      throw new Error('SQLite configuration accessors are not supported')
  }
}
function location<Database extends object>(options: SqliteLocation<Database>): Source<Database> {
  if ((options.path === undefined) === (options.database === undefined))
    throw new Error('Supply exactly one SQLite path or database')
  if (options.database !== undefined) {
    validateDatabase(options.database)
    return Object.freeze({ ownership: 'borrowed', database: options.database })
  }
  const path = options.path
  if (path === undefined) throw new Error('A file-owned SQLite connection requires a path')
  requireName(path, 'sqlite.path')
  if (path.includes('\0') || path.startsWith('file:') || path === ':memory:') {
    throw new Error(
      'Use a local file path; for an in-memory database supply an explicitly migrated native database handle'
    )
  }
  return Object.freeze({ ownership: 'owned', path: resolve(path) })
}
function integer(value: number, name: string, minimum: number): number {
  requireInteger(value, name, minimum)
  if (value > 2_147_483_647)
    throw new Error(`${name} exceeds the supported timer/SQLite integer range`)
  return value
}

export function sqliteConnection<Database extends object>(
  options: SqliteOptions<Database>,
  host: SqliteHost<Database>
): MqConnection {
  try {
    ...
    if (!configurePragmas && options.busyTimeoutMs !== undefined)
      throw new Error('busyTimeoutMs requires explicit pragma configuration on a borrowed handle')
    ...
    return defineConnection(
      {
        ...
        boundary: source.ownership === 'owned' ? source.path : source.database,
        scope: namespace,
        requirements
      },
      async (token) => {
        const database =
          source.ownership === 'owned' ? await host.open(source.path) : source.database
        let closed = false
        const release = async (): Promise<void> => {
          if (closed || source.ownership === 'borrowed') return
          closed = true
          host.close(database)
        }
        try {
          const native = validateDatabase(database)
          // File ownership grants lifecycle/configuration ownership, never permission to migrate.
          if (source.ownership === 'owned') native.exec('PRAGMA journal_mode = WAL')
          const layer = SqliteJobStore.layerFor(token, {
            database: native,
            namespace,
            configurePragmas,
            busyTimeoutMs,
          ...
          try {
            ...
            throw new AggregateError(
              [cause, cleanupCause],
              'SQLite acquisition and cleanup failed',
              { cause }
            )
          }
          throw cause
...
async function withDatabase<Database extends object, Value>(
  options: SqliteLocation<Database>,
  host: SqliteHost<Database>,
  operation: (database: SqliteDatabase) => Value
): Promise<Value> {
  assertOptions(options, ['path', 'database'])
  const source = location(options)
  const database = source.ownership === 'owned' ? await host.open(source.path) : source.database
  let result: Value
  try {
    result = operation(validateDatabase(database))
  } catch (cause) {
    if (source.ownership === 'owned') {
      try {
        host.close(database)
      } catch (cleanupCause) {
        throw new AggregateError([cause, cleanupCause], 'SQLite operation and cleanup failed', {
          cause
        })
      }
    }
  ...
  if (source.ownership === 'owned') host.close(database)
  return result
}
export async function migrateSqliteDatabase<Database extends object>(
  options: SqliteLocation<Database>,
  host: SqliteHost<Database>
): Promise<SqliteMigrationReport> {
  try {
    return await withDatabase(options, host, (database) => {
      ...
      return Object.freeze({
        ...
        applied: Object.freeze([...report.applied])
      })
    })
  } catch (cause) {
    throw new MqConnectionException('<sqlite-migrations>', 'migrate', { cause })
  }
}
export async function validateSqliteDatabase<Database extends object>(
  options: SqliteLocation<Database>,
  host: SqliteHost<Database>
): Promise<void> {
  try {
    await withDatabase(options, host, (database) => {
      SqliteMigrator.validate(database)
    })
  } catch (cause) {
    throw new MqConnectionException('<sqlite-migrations>', 'probe', { cause })
  }
}
