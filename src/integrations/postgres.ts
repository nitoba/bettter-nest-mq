...
function withOutbox(resource: AcquiredConnection, pool: Pool, schema: string, namespace: string, enabled: boolean): AcquiredConnection {
...
function createPostgresConnection(options: PostgresConnectionOptions): MqConnection {
  ...
  if (outbox !== true && outbox !== false) throw new MqConnectionException('<postgres>', 'configuration')
  ...
  if (options.pool !== undefined) {
    ...
    validatePool(pool)
    return defineConnection({ adapter: 'postgres', ownership: 'borrowed', boundary: pool, scope, requirements }, (token) => withOutbox({
      layer: PostgresJobStore.layerFor(token, { pool: postgresJsonPool(pool), schema, namespace, validateSchema: validate })
    }, pool, schema, namespace, outbox))
  }
  ...
  return defineConnection({ adapter: 'postgres', ownership: 'owned', boundary: connectionString, scope, requirements }, async (token) => {
    ...
    const onIdleError = (): void => { logger.warn('An idle PostgreSQL client disconnected; the pool will replace it on demand') }
    pool.on('error', onIdleError)
    const release = async (): Promise<void> => { await pool.end(); pool.removeListener('error', onIdleError) }
    try {
      return withOutbox({ layer: PostgresJobStore.layerFor(token, { pool: postgresJsonPool(pool), schema, namespace, validateSchema: validate }), release }, pool, schema, namespace, outbox)
    } catch (cause) {
      try { await release() }
...
export type { PostgresOutboxClient, PostgresOutboxTransaction, PostgresOutboxCallback, PostgresOutboxParameter } from './postgres-outbox.ts'
