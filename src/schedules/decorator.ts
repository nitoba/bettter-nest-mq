...
export function copySchedule<Input>(options: ScheduleOptions<Input>): ScheduleOptions<Input> {
  try {
    ...
    if (options.misfire !== undefined) {
      ...
      else if (options.misfire.strategy !== 'skip' && options.misfire.strategy !== 'run-once')
...
export function resolveScheduleOptions(
  ...
  if (resolved.mode !== 'validate' && resolved.mode !== 'reconcile')
    throw new MqScheduleException('definition', 'schedules.mode must be validate or reconcile')
