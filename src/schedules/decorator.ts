...
function assertFields<Options extends object>(options: Options, allowed: readonly string[]): void {
  const prototype = Object.getPrototypeOf(options)
  if (prototype !== Object.prototype && prototype !== null)
    throw new MqScheduleException('definition', 'Schedule options must be plain data objects')
  for (const key of Reflect.ownKeys(options)) {
    if (key !== String(key) || !allowed.includes(String(key)))
      throw new MqScheduleException('definition', `Unsupported schedule option ${String(key)}`)
    const descriptor = Object.getOwnPropertyDescriptor(options, key)
    if (descriptor?.get !== undefined || descriptor?.set !== undefined)
      throw new MqScheduleException('definition', 'Schedule option accessors are not supported')
  }
}
export function scheduleName(value: string, field: string): string {
  requireName(value, field)
  if (value.length > maxScheduleIdentityLength || value.includes('\0'))
    throw new MqScheduleException(
      'definition',
      `${field} exceeds the schedule identity limit or contains NUL`
    )
  return value
}
...
export function copySchedule<Input>(options: ScheduleOptions<Input>): ScheduleOptions<Input> {
  try {
    ...
    if ((options.cron === undefined) === (options.everyMs === undefined))
      throw new Error('Exactly one of cron or everyMs is required')
    ...
    if (options.overlap !== undefined && options.overlap !== 'skip' && options.overlap !== 'allow')
      throw new Error('Invalid overlap policy')
    if (options.misfire !== undefined) {
      assertFields(options.misfire, ['strategy', 'maxOccurrences'])
      if (options.misfire.strategy === 'catch-up') {
        requireInteger(options.misfire.maxOccurrences, 'misfire.maxOccurrences', 1)
        if (options.misfire.maxOccurrences > MAX_OCCURRENCES_PER_TICK)
          throw new Error('Catch-up exceeds the pinned store occurrence limit')
      } else {
        assertFields(options.misfire, ['strategy'])
        if (options.misfire.strategy !== 'skip' && options.misfire.strategy !== 'run-once')
          throw new Error('Invalid misfire policy')
      }
    }
    if (options.metadata !== undefined) {
      assertFields(options.metadata, Object.keys(options.metadata))
      for (const value of Object.values(options.metadata)) {
        if (value !== String(value)) throw new Error('Schedule metadata values must be strings')
      }
    }
    const text = JSON.stringify(options.payload)
    if (text === undefined) throw new Error('Schedule payload must be JSON input')
    const payload = JSON.parse(text)
    if (!isDeepStrictEqual(payload, options.payload))
      throw new Error('Schedule input must have a lossless JSON representation')
    freezeJson(payload)
    let result: ScheduleOptions<Input> = { ...options, payload }
    if (options.retry !== undefined) result = { ...result, retry: copyRetry(options.retry) }
    if (options.metadata !== undefined)
      result = { ...result, metadata: Object.freeze({ ...options.metadata }) }
    if (options.misfire !== undefined)
      result = { ...result, misfire: Object.freeze({ ...options.misfire }) }
...
export function Schedule<Input extends JobJsonValue>(
  ...
  return (target, key) => {
    if (!(target instanceof QueueService) || key !== String(key))
      throw new MqScheduleException(
        'definition',
        '@Schedule requires a string-named QueueService job property'
      )
    const own: ReadonlyMap<string, readonly ScheduleOptions[]> =
      Reflect.getOwnMetadata(SCHEDULES, target) ?? new Map()
    const previous = own.get(String(key)) ?? []
    if (previous.some((entry) => entry.key === copied.key && entry.group === copied.group))
      throw new MqScheduleException('definition', 'Duplicate schedule decorator address')
...
export function scheduleMetadata(
  ...
  while (prototype !== null) {
    const map: ReadonlyMap<string, readonly ScheduleOptions[]> | undefined = Reflect.getOwnMetadata(
      SCHEDULES,
      prototype
    )
...
export function assertScheduleProperties(
  queue: QueueService,
  ...
  while (prototype !== null) {
    ...
    for (const property of map?.keys() ?? []) {
      if (!properties.has(property))
        throw new MqScheduleException(
          'definition',
          `@Schedule on ${property} has no registered @Job`
        )
    }
    prototype = Object.getPrototypeOf(prototype)
  }
...
): Readonly<Required<MqScheduleOptions>> {
  try {
    assertFields(options, [
      'mode',
      'group',
    ...
    const resolved = {
      ...
      group: options.group ?? 'nestjs/schedules',
      sweepIntervalMs: options.sweepIntervalMs ?? 1_000,
      batchSize: options.batchSize ?? 100,
      maxStoreRetries: options.maxStoreRetries ?? 3,
      retryDelayMs: options.retryDelayMs ?? 25
    }
    if (resolved.mode !== 'validate' && resolved.mode !== 'reconcile')
      throw new Error('schedules.mode must be validate or reconcile')
    ...
    if (resolved.sweepIntervalMs > MAX_TIMER_DELAY || resolved.retryDelayMs > MAX_TIMER_DELAY)
      throw new Error('Scheduler delays exceed the JavaScript timer range')
    return Object.freeze(resolved)
  } catch (cause) {
    if (cause instanceof MqScheduleException) throw cause
    throw new MqScheduleException('definition', 'Invalid scheduler configuration', { cause })
  }
}
