...
function assertFields<Options extends object>(options: Options, allowed: readonly string[]): void {
  ...
  if (prototype !== Object.prototype && prototype !== null) throw new MqScheduleException('definition', 'Schedule options must be plain data objects')
  for (const key of Reflect.ownKeys(options)) {
    if (key !== String(key) || !allowed.includes(String(key))) throw new MqScheduleException('definition', `Unsupported schedule option ${String(key)}`)
    const descriptor = Object.getOwnPropertyDescriptor(options, key)
    if (descriptor?.get !== undefined || descriptor?.set !== undefined) throw new MqScheduleException('definition', 'Schedule option accessors are not supported')
  }
}
...
export function copySchedule<Input>(options: ScheduleOptions<Input>): ScheduleOptions<Input> {
  try {
    assertFields(options, ['key', 'group', 'payload', 'cron', 'everyMs', 'timeZone', 'metadata', 'retry', 'priority', 'timeoutMs', 'misfire', 'overlap'])
    ...
    if (options.misfire !== undefined) {
      ...
      if (options.misfire.strategy === 'catch-up') {
        ...
        if (options.misfire.maxOccurrences > MAX_OCCURRENCES_PER_TICK) throw new Error('Catch-up exceeds the pinned store occurrence limit')
      } else {
        assertFields(options.misfire, ['strategy'])
        if (options.misfire.strategy !== 'skip' && options.misfire.strategy !== 'run-once') throw new Error('Invalid misfire policy')
      }
    }
...
export function resolveScheduleOptions(options: MqScheduleOptions = {}): Readonly<Required<MqScheduleOptions>> {
  try {
    assertFields(options, ['mode', 'group', 'sweepIntervalMs', 'batchSize', 'maxStoreRetries', 'retryDelayMs'])
    const resolved = {
    ...
    if (resolved.mode !== 'validate' && resolved.mode !== 'reconcile') throw new Error('schedules.mode must be validate or reconcile')
    ...
    if (resolved.sweepIntervalMs > MAX_TIMER_DELAY || resolved.retryDelayMs > MAX_TIMER_DELAY) throw new Error('Scheduler delays exceed the JavaScript timer range')
