...
export class MqJobException extends Error {
  ...
  constructor(
    readonly operation: JobOperationName,
    message: string,
...
export class JobWaitTimeoutException extends Error {
  ...
  constructor(
    ...
    super(
      `Waiting for job ${JSON.stringify(jobId)} exceeded ${timeoutMs}ms; the job was not cancelled`
    )
...
export class JobWaitAbortedException extends Error {
  ...
  constructor(
    ...
    super(
      `Waiting for job ${JSON.stringify(jobId)} was aborted; the job was not cancelled`,
      options
    )
