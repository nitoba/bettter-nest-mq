...
type JobProperty<Queue extends QueueService> = {
...
export function Worker(options: WorkerOptions): ClassDecorator {
  ...
  return (target) => {
    if (Reflect.hasOwnMetadata(WORKER, target))
      throw new ContractDefinitionException('Duplicate @Worker decorator')
    Reflect.defineMetadata(WORKER, snapshot, target)
  }
}

export function Process<Queue extends QueueService>(
  queue: Type<Queue>,
  property: JobProperty<Queue>,
  options: ProcessOptions = {}
): MethodDecorator {
  if (!(queue.prototype instanceof QueueService))
    throw new ContractDefinitionException('@Process requires a QueueService class')
  if (options.concurrency !== undefined)
    requireInteger(options.concurrency, 'process.concurrency', 1)
  ...
  return (target, method, descriptor) => {
    if (method !== String(method) || !(descriptor.value instanceof Function))
      throw new ContractDefinitionException('@Process requires a string-named method')
    const methods: ReadonlyMap<string, ProcessMetadata> =
      Reflect.getOwnMetadata(PROCESSORS, target) ?? new Map()
    if (methods.has(String(method)))
      throw new ContractDefinitionException('Duplicate @Process decorator')
...
function parameter(kind: JobParameter): ParameterDecorator {
  return (target, method, index) => {
    if (method === undefined || method !== String(method))
      throw new ContractDefinitionException('MQ parameters are only supported on named methods')
    const parameters: ReadonlyMap<
      string,
      ReadonlyMap<number, JobParameter>
    > = Reflect.getOwnMetadata(PARAMETERS, target) ?? new Map()
    const current = new Map(parameters.get(String(method)))
    if (current.has(index))
      throw new ContractDefinitionException('Duplicate MQ parameter decorator')
...
export function JobContext(): ParameterDecorator {
  return parameter('context')
}
export function workerMetadata<Target extends object>(target: Target): WorkerOptions | undefined {
  return Reflect.getOwnMetadata(WORKER, target)
}

export function processMetadata<Target extends object>(
  target: Target
): ReadonlyMap<string, ProcessMetadata> {
  ...
  for (const prototype of hierarchy.reverse()) {
    const methods: ReadonlyMap<string, ProcessMetadata> | undefined = Reflect.getOwnMetadata(
      PROCESSORS,
      prototype
    )
...
export function parameterMetadata<Target extends object>(
  target: Target,
  method: string
): ReadonlyMap<number, JobParameter> {
  let current = target
  while (current !== null) {
    if (Object.getOwnPropertyDescriptor(current, method) !== undefined) {
      ...
      const parameters: ReadonlyMap<string, ReadonlyMap<number, JobParameter>> | undefined =
        Reflect.getOwnMetadata(PARAMETERS, current)
