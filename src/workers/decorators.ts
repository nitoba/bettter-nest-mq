...
export function Worker(options: WorkerOptions): ClassDecorator {
  ...
  if ((options.heartbeatIntervalMs ?? Math.floor(lease / 3)) >= lease)
...
export function processMetadata<Target extends object>(
  ...
  return Reflect.getMetadata(PROCESSORS, target) ?? new Map()
}
export function parameterMetadata<Target extends object>(
  target: Target,
  method: string
): ReadonlyMap<number, JobParameter> {
  const parameters: ReadonlyMap<string, ReadonlyMap<number, JobParameter>> = Reflect.getMetadata(
    PARAMETERS,
    target
  ) ?? new Map()
