...
const internalPackages = ['better-effect', 'better-result', 'better-effect-mq', 'better-effect-mq-postgres', 'better-effect-mq-outbox']
...
try {
  ...
  for (const compiler of ['typescript-minimum', 'typescript']) {
    ...
    for (const name of internalPackages) assert.equal(name in manifest.dependencies || name in manifest.devDependencies, false, `Consumer must not declare internal dependency ${name}`)
    ...
    const integratedManifest = {
      ...manifest,
      devDependencies: { ...manifest.devDependencies, zod: await installedVersion('zod'), pg: await installedVersion('pg'), '@types/pg': await installedVersion('@types/pg') }
    }
    for (const name of internalPackages) assert.equal(name in integratedManifest.dependencies || name in integratedManifest.devDependencies, false, `Integrated consumer must not declare internal dependency ${name}`)
    ...
    console.log(`Packed consumers passed with TypeScript ${version}; no engine or adapter dependencies were declared by the application`)
  }
