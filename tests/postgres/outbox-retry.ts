...
import { Pool } from 'pg'
import { z } from 'zod'
import { Job, MqModule, MqOutboxException, MqOutboxService, Queue, QueueService, type OutboxSnapshot } from '../../src/index.ts'
...
class ProtocolQueue extends QueueService {
  ...
  readonly job = this.job({ payload: z.string(), result: z.string() })
}
...
export async function verifyOutboxRetryProtocol(connectionString: string): Promise<void> {
  ...
  const app = await Test.createTestingModule({ imports: [
    MqModule.forRoot({ connections: { primary: postgres({ pool, schema, outbox: true }) }, execution: { workers: false, outboxPublisher: false } }),
  ...
  try {
    ...
    await postgresOutbox(service).transaction({ id: 'record', job, attempts: 1 }, async () => undefined)
    await outboxCoordinator(service).withSource('primary', async (store) => {
      const owner = valueOf(makeOutboxWorkerId('test-publisher'))
      const claims = valueOf(await store.claim({ owner, limit: 1, nowMs: Date.now(), leaseDurationMs: 30_000 }))
      const active = claims[0]
      assert.ok(active)
      const snapshot = await service.get('primary', active.id)
      assert.ok(snapshot)
      await assert.rejects(service.retryFailed('primary', active.id, { expected: expected(snapshot), attempts: 2 }), MqOutboxException)
      const failedAt = Math.max(Date.now(), active.updatedAtMs)
      valueOf(await store.markFailed({ id: active.id, leaseToken: active.leaseToken, nowMs: failedAt,
        failure: { kind: 'store-permanent', message: 'Reproducible failure', retryable: false, recordedAtMs: failedAt }
      }))
      const failed = valueOf(await store.get(active.id))
      assert.ok(failed)
      const firstExpected = expected(failed)
      const pending = await service.retryFailed('primary', active.id, { expected: firstExpected, attempts: 2 })
      assert.equal(pending.state, 'pending')
      const oldPublished = await store.markPublished({ id: active.id, leaseToken: active.leaseToken, nowMs: pending.updatedAtMs + 1 })
      assert.ok(Result.isError(oldPublished), 'Old lease cannot acknowledge an administratively retried publication')
      const oldHeartbeat = await store.heartbeat({ id: active.id, leaseToken: active.leaseToken, nowMs: pending.updatedAtMs + 1, leaseDurationMs: 30_000 })
      assert.ok(Result.isError(oldHeartbeat))
      assert.equal(valueOf(await store.get(active.id))?.state, 'pending')
      const second = valueOf(await store.claim({ owner, limit: 1, nowMs: Math.max(Date.now(), pending.runAtMs), leaseDurationMs: 30_000 }))[0]
      assert.ok(second)
      ...
      valueOf(await store.markFailed({ id: second.id, leaseToken: second.leaseToken, nowMs: secondAt,
        failure: { kind: 'store-permanent', message: 'Second publication failure', retryable: false, recordedAtMs: secondAt }
      }))
      await assert.rejects(service.retryFailed('primary', second.id, { expected: firstExpected, attempts: 2 }), MqOutboxException)
      const current = valueOf(await store.get(active.id))
      assert.ok(current)
      const desired = retryOutboxRecord(current, { expected: expected(current), attempts: 1 }, Date.now())
      ...
      await pool.query(`UPDATE "${schema}".better_effect_mq_outbox SET target='different-target' WHERE id=$1`, [current.id])
    ...
    console.log('PASS actual PostgreSQL active/old-lease fencing, repeated recovery guards and full-content compare-and-swap')
  } finally {
    try { await app.close() }
