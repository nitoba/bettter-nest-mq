...
export async function verifyTypedFlowFailure(connectionString: string): Promise<void> {
  ...
  try {
    ...
    const jobs = valueOf(
      await runtime.run(() => Effect.gen(async function* () {
    ...
    try {
      ...
      const parent = valueOf(await jobs.enqueue({
        job: identity, payload: { probe: true }, runAt: now, now, attemptsMax: 2
      }))
      const claimed = valueOf(await jobs.claim({
        queue, accepted: [identity], workerId: valueOf(makeWorkerId('probe')),
        limit: 1, leaseDurationMs: 60_000, now: now + 1
      })).jobs[0]
      assert.ok(claimed)
      const children = ['bad', 'good'].map((key) => {
        const id = valueOf(makeFlowChildId({
          parentStoreKey: 'default', flowId: parent.job.id, childKey: key
        }))
        return {
          childKey: key, name: 'child', version: 1, storeKey: 'default', childJobId: id,
          request: valueOf(makePreparedEnqueue({
            protocolVersion: 1, identity: { queue: 'failure-probe', name: 'child', version: 1 },
            id, payload: { probe: true }, metadata: {}, priority: 0,
            attemptsMax: 1, runAt: now + 2, now: now + 2
          }))
        }
      })
      valueOf(await flows.fanOut({
        flowId: parent.job.id, flowName: 'failure-probe', parentStoreKey: 'default',
        depth: 1, leaseToken: claimed.leaseToken, failFast: false, children, now: now + 2
      }))
      const reconciled = await flows.reconcile({
        flowId: parent.job.id, now: now + 3,
        observations: [
          {
            childKey: 'bad', state: 'failed',
            failure: {
              kind: 'typed', code: 'handler-failure', message: 'Handler returned a typed failure',
              data: { code: 'child-failed' }, retryable: false, recordedAt: now + 3
            }
          },
      ...
      const recorded = await flows.recordChildResults({
        flowId: parent.job.id, reports, now: now + 4
      })
      ...
      assert.deepEqual(value.parent.flow, {
        flowName: 'failure-probe', failFast: false, pending: 0, completed: 1, failed: 1, cancelled: 0
      })
      console.log('PASS published PostgreSQL mixed typed-failure/success observations release the parent')
    } finally {
      await flows.dispose()
    }
  } finally {
