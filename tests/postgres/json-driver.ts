...
try {
  const result = await view.query(
    'SELECT $1::json AS json_text,$1::jsonb AS jsonb_text,$2::jsonb AS json_null,NULL::jsonb AS sql_null,$3::integer AS n,$4::text AS plain',
    [JSON.stringify('null'), 'null', 42, '123']
  )
  ...
  try {
    ...
    await client.query('INSERT INTO json_driver_probe(value) VALUES ($1::jsonb)', [
      JSON.stringify('123')
    ])
    assert.deepEqual((await client.query('SELECT value FROM json_driver_probe')).rows, [
      { value: '"123"' }
    ])
    assert.deepEqual((await client.query('SELECT pg_backend_pid() AS pid')).rows, backend.rows)
    await client.query('ROLLBACK')
    assert.deepEqual(
      (await client.query("SELECT to_regclass('pg_temp.json_driver_probe') AS relation")).rows,
      [{ relation: null }]
    )

    const notification = new Promise<string>((resolve, reject) => {
      onNotification = (message) => {
        if (message.channel === channel) resolve(message.payload ?? '')
      }
      client.on('notification', onNotification)
      timer = setTimeout(
        () => reject(new Error('Native LISTEN notification did not arrive')),
        3_000
      )
    })
  ...
  console.log(
    'PASS adapter JSON/JSONB versus SQL NULL, native application parsing, same-client transaction rollback, listener delegation and error cleanup'
  )
} finally {
  await pool.end()
}
