...
export async function verifyOrdinaryRecovery(connectionString: string): Promise<void> {
  ...
  try {
    ...
    await session.withStore('primary', async (raw) => {
      ...
      const now = Date.now()
      const saved = await raw.enqueue({ job: identity, payload: { message: 'retry me' }, attemptsMax: 2, now, runAt: now })
      if (Result.isError(saved)) throw saved.error
      const claimed = await raw.claim({ queue: queue.value, accepted: [identity], workerId: workerId.value, limit: 1, leaseDurationMs: 20, now })
      if (Result.isError(claimed)) throw claimed.error
      assert.equal(claimed.value.jobs.length, 1)
      console.log('Ordinary recovery regression: actual PostgreSQL job acquired, checking expired lease')
