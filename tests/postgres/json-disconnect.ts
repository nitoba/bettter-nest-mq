...
try {
  ...
  assert.notEqual(second.rows[0]?.pid, first.rows[0]?.pid, 'Like native pool.query, the adapter view must discard a client released after query failure')
  ...
  while (!terminated) {
    ...
    const result = await control.query<{ terminated: boolean }>(
      "SELECT pg_terminate_backend(pid) AS terminated FROM pg_stat_activity WHERE application_name=$1 AND state='active' AND query='SELECT pg_sleep(10)'", [tag]
    )
    terminated = result.rows.some((row) => row.terminated)
    if (!terminated) await sleep(10)
  }
  await rejected
  assert.deepEqual((await view.query('SELECT $1::jsonb AS value', [JSON.stringify('123')])).rows, [{ value: '"123"' }])
  assert.deepEqual((await pool.query('SELECT $1::jsonb AS value', [JSON.stringify('123')])).rows, [{ value: '123' }])
  assert.equal(pool.waitingCount, 0)
  console.log('PASS query failure discards its client; active backend termination rejects without process crash and the same native pool remains usable')
} finally {
  try { await pool.end() }
