...
import { Pool } from 'pg'
import {
  LeaseLostError, Queue as EngineQueue, QueueControls as EngineControls,
...
export async function verifyPostgresControlledClock(connectionString: string): Promise<void> {
  ...
  try {
    ...
    await session.withStore('primary', async (raw) => {
      ...
      const identity = { queue: valueOf(makeQueueName('postgres-clock')), name: valueOf(makeJobName('task')), version: 1 }
      const workerId = valueOf(makeWorkerId('postgres-clock-worker'))
      const policy = valueOf(await controlled.reconcile(EngineControls.registry({ group: 'clock-test', controls: [EngineControls.define(EngineQueue.define('postgres-clock'), { globalConcurrency: 1 })] })))
      const revision = policy.records[0]?.revision
      assert.ok(revision)
      const view = dispatchStore(raw, queues)
      const now = Date.now()
      const saved = valueOf(await raw.enqueue({ job: identity, payload: { value: 'payload' }, attemptsMax: 2, now, runAt: now }))
      const claim = { queue: identity.queue, accepted: [identity], workerId, controlsRevision: revision, limit: 1, leaseDurationMs: 60_000 }
      const job = valueOf(await controlled.claimControlled({ ...claim, now: now + 1 })).jobs[0]
      assert.ok(job)
      valueOf(await raw.heartbeat({ leases: [{ jobId: job.id, leaseToken: job.leaseToken }], now: now + 1_000, leaseDurationMs: 60_000 }))
      const request = { jobId: job.id, leaseToken: job.leaseToken, startedAt: now + 1, now: now + 10, outcome: { type: 'complete' as const, result: { value: 'persisted' } } }
      ...
      valueOf(await raw.enqueue({ job: identity, payload: { value: 'cancel' }, attemptsMax: 2, now: later, runAt: later }))
      const active = valueOf(await controlled.claimControlled({ ...claim, now: later + 1 })).jobs[0]
      assert.ok(active)
      valueOf(await raw.heartbeat({ leases: [{ jobId: active.id, leaseToken: active.leaseToken }], now: later + 1_000, leaseDurationMs: 60_000 }))
      ...
      const expired = await view.settle({ jobId: active.id, leaseToken: active.leaseToken, now: later + 100_000, outcome: { type: 'complete', result: { value: 'late' } } })
      assert.ok(Result.isError(expired))
      assert.ok(expired.error instanceof LeaseLostError)
      valueOf(await controlled.recoverStalledControlled({ queue: identity.queue, controlsRevision: revision, now: later + 100_000, maxStalledCount: 1 }))
      const permits = await pool.query<{ count: number }>(`SELECT count(*)::integer AS count FROM "${schema}".better_effect_mq_controlled_permits`)
      assert.equal(permits.rows[0]?.count, 0)
    })
    console.log('PASS PostgreSQL clock races: one durable completion, idempotent acknowledgment, cooperative cancellation and expired-lease fencing')
  } finally {
