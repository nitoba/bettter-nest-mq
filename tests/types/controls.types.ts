...
const timestamp = zodCodec(z.codec(z.iso.datetime(), z.date(), { decode: (input) => new Date(input), encode: (date) => date.toISOString() }))
class Typed extends QueueService {
  readonly task = this.job({ payload: timestamp, result: z.string(), dispatchKey: (payload) => payload.toISOString() })
...
QueueControls({ globalConcurrency: 3, perKeyConcurrency: 1, rateLimit: { max: 10, durationMs: 1_000 } })
