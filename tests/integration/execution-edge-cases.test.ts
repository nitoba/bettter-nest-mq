...
test('inherited processors remain registered when a subclass adds another decorated method', async () => {
  ...
  try {
    ...
    expect(await queue.first.execute('value', { wait: { timeoutMs: 1_000, pollIntervalMs: 5 } })).toBe('base:value')
    expect(await queue.second.execute('value', { wait: { timeoutMs: 1_000, pollIntervalMs: 5 } })).toBe('second')
