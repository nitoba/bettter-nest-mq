...
import { MemoryJobStore, Queue as EngineQueue, QueueControls as EngineControls, makeQueueName } from 'better-effect-mq'
import { Result } from 'better-result'
import { z } from 'zod'
import { ContractDefinitionException, Job, JobData, MqModule, MqQueueControlsService, Process, Queue, QueueControls, QueueControlsException, QueueService, Worker, type MqModuleOptions } from '../../src/index.ts'
...
class LimitedQueue extends QueueService {
  ...
  readonly task = this.job({ payload: z.object({ tenant: z.string(), value: z.int() }), result: z.int(), dispatchKey: (payload) => payload.tenant })
...
function sharedStore() {
  ...
  let acquired = 0
  let released = 0
  const connection = defineConnection({ adapter: 'memory', ownership: 'borrowed', boundary: store, scope: 'shared-control-test' }, (token) => ({
    layer: Layer.scoped(token, () => { acquired += 1; return store }, () => { released += 1 })
  }))
  return { store, connection, counts: () => ({ acquired, released }) }
}

async function createApp(fixture: ReturnType<typeof sharedStore>, controls?: MqModuleOptions['controls']) {
  const options: MqModuleOptions = controls === undefined
  ...
  return Test.createTestingModule({ imports: [MqModule.forRoot(options), MqModule.forFeature([LimitedQueue])] }).compile()
}
...
test('explicit deployment reconciliation persists policy, keeps its revision, and allows read-only replicas', async () => {
  ...
  try {
    ...
    expect(first).toMatchObject({ queue: 'limited', connection: 'primary', enabled: true, globalConcurrency: 2, perKeyConcurrency: 1 })
...
test('policy drift and group ownership conflicts fail without rewriting persisted limits', async () => {
  const fixture = sharedStore()
  const registry = EngineControls.registry({ group: 'foreign-owner', controls: [EngineControls.define(EngineQueue.define('limited'), { globalConcurrency: 7 })] })
  ...
  expect(persisted.value).toMatchObject({ group: 'foreign-owner', revision: 1, globalConcurrency: 7 })
})
...
test('derived keys are used by prepare and batches, cannot be overridden, and missing manual keys fail before publication', async () => {
  ...
  try {
    ...
    await assert.rejects(queue.task.enqueue({ tenant: 'A', value: 1 }, { dispatchKey: 'B' }), ContractDefinitionException)
    ...
    await assert.rejects(queue.task.enqueueMany([
    ...
    expect((await queue.task.enqueueMany([{ payload: { tenant: 'A', value: 1 } }, { payload: { tenant: 'B', value: 2 } }]))).toHaveLength(2)
...
class LimitedWorker {
  ...
  async run(@JobData() payload: { tenant: string; value: number }) {
    ...
    try { await this.gates.release.promise; return payload.value }
    finally { this.gates.active.delete(payload.tenant) }
  }
}
...
test('the real supervisor reaches controlled claims and settles permits rather than falling back to plain claims', async () => {
  ...
  const app = await Test.createTestingModule({ imports: [
    MqModule.forRoot({ connections: { primary: fixture.connection }, controls: { mode: 'reconcile' } }), MqModule.forFeature([LimitedQueue])
  ], providers: [Gates, LimitedWorker] }).compile()
  const gates = app.get(Gates)
  try {
    await app.init()
    const queue = app.get(LimitedQueue)
    const ids = await queue.task.enqueueMany([
      { payload: { tenant: 'A', value: 1 } }, { payload: { tenant: 'A', value: 2 } },
      { payload: { tenant: 'B', value: 3 } }, { payload: { tenant: 'C', value: 4 } }
    ])
    await gates.twoKeys.promise
    expect(new Set(gates.started.slice(0, 2)).size).toBe(2)
    gates.release.resolve()
    expect(await Promise.all(ids.map((id) => queue.task.awaitResult(id, { timeoutMs: 2_000, pollIntervalMs: 5 })))).toEqual([1, 2, 3, 4])
  } finally { gates.release.resolve(); await app.close() }
})
