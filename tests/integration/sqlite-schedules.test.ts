...
function connection(database: Database, enabled = true): MqConnection {
  // Exercise the public JavaScript boundary against the original missing-option baseline.
  return sqlite({
...
test('SQLite schedules preserve scalar payloads, namespace and revisions across administrative writes', async () => {
...
test('SQLite replicas validate without overwriting operator pauses, cursors or omitted definitions', async () => {
  const database = new Database(':memory:')
  await migrateSqlite({ database })
  const app = await application(connection(database))
  let saved
...
test('SQLite scheduler emits one durable occurrence and native event waits observe its result', async () => {
  const database = new Database(':memory:')
  await migrateSqlite({ database })
  const app = await application(connection(database), {
...
test('a validating SQLite replica cannot silently deploy missing schedule declarations', async () => {
  const database = new Database(':memory:')
  await migrateSqlite({ database })
...
for (const owned of [true, false]) {
  test(`failed SQLite schedule acquisition cleans up the ${owned ? 'owned' : 'borrowed'} handle`, async () => {
    const database = new Database(':memory:')
    await migrateSqlite({ database })
