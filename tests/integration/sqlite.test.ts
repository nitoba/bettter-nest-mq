...
import { Job, JobData, JobContext, JobFailureException, MqConnectionException, MqConnectionsService, MqModule, Process, Queue, QueueService, Retry, Worker, type MqConnection, type JobExecutionContext } from '../../src/index.ts'
import { z } from 'zod'
...
class Jobs extends QueueService {
  ...
  readonly retry = this.job({ payload: z.string(), result: z.string(), failure: z.object({ retry: z.boolean() }), retryable: (value) => value.retry })
  @Job({ name: 'date', version: 1 })
  readonly date = this.job({ payload: zodCodec(z.codec(z.iso.datetime(), z.date(), { decode: (value) => new Date(value), encode: (date) => date.toISOString() })), result: z.string() })
}
@Worker({ name: 'sqlite', concurrency: 2, pollIntervalMs: 5 })
class Processor {
  @Process(Jobs, 'echo')
  echo(@JobData() value: z.output<ReturnType<typeof z.json>>) { return value }
  @Process(Jobs, 'retry')
  ...
  @Process(Jobs, 'date')
  date(@JobData() value: Date) { assert.ok(value instanceof Date); return value.toISOString() }
}
async function application(connection: MqConnection, workers = false) {
  const app = await Test.createTestingModule({ imports: [MqModule.forRoot({ connections: { primary: connection }, execution: { workers } }), MqModule.forFeature([Jobs])], providers: [Processor] }).compile()
  return app
}
...
test('borrowed native handles remain open and their pragmas unchanged by default', async () => {
  ...
  try {
    ...
    await app.init()
    expect(app.get(MqConnectionsService).connections()[0]).toMatchObject({ adapter: 'sqlite', ownership: 'borrowed' })
    await app.close()
...
test('owned database cleanup happens exactly once after failed startup and after normal shutdown', async () => {
  ...
  const host: SqliteHost<Database> = {
    ...
    close(database) { closed += 1; database.close(true) }
  }
...
test('file-backed public jobs survive producer exit and preserve JSON, retries and Date codecs', async () => {
  ...
  const path = join(directory, 'jobs.db')
  const values = ['123', 'true', 'null', '{"value":1}', '', 'ação', 0, 5, false, null, ['array'], { value: 'object' }]
  try {
    ...
    try {
      ...
      expect(await jobs.echo.enqueue('123', { idempotencyKey: 'unique' })).toBe(await jobs.echo.enqueue('123', { idempotencyKey: 'unique' }))
    ...
    const worker = await application(sqlite({ path, namespace: 'durable', pollIntervalMs: 5 }), true)
    await worker.init()
    try {
      const jobs = worker.get(Jobs)
      expect(await Promise.all(ids.map((id) => jobs.echo.awaitResult(id, { timeoutMs: 3000, pollIntervalMs: 5 })))).toEqual(values)
      const retryId = await jobs.retry.enqueue('retried')
      expect(await jobs.retry.awaitResult(retryId, { timeoutMs: 3000, pollIntervalMs: 5 })).toBe('retried')
      expect((await jobs.retry.attempts(retryId)).map((attempt) => attempt.outcome)).toEqual(['retried', 'completed'])
      expect(await jobs.date.execute('2026-09-15T12:00:00.000Z', { wait: { timeoutMs: 3000, pollIntervalMs: 5 } })).toBe('2026-09-15T12:00:00.000Z')
    } finally { await worker.close() }
    const reader = await application(sqlite({ path, namespace: 'durable' }))
    await reader.init()
    try { expect((await reader.get(Jobs).echo.poll(ids[0]!))?.result).toBe('123') }
    finally { await reader.close() }
    const isolated = await application(sqlite({ path, namespace: 'another' }))
    await isolated.init()
    try { expect(await isolated.get(Jobs).echo.poll(ids[0]!)).toBeUndefined() }
    finally { await isolated.close() }
  } finally { await rm(directory, { recursive: true, force: true }) }
})

test.each([
  {}, { path: '' }, { path: ':memory:' }, { path: 'file:jobs.db' }, { path: 'jobs.db', outbox: true },
  { path: 'jobs.db', pollIntervalMs: 0 }, { path: 'jobs.db', busyTimeoutMs: -1 },
  { path: 'jobs.db', pollIntervalMs: 2_147_483_648 }, { path: 'jobs.db', configurePragmas: 'yes' }
...
test('configuration getters are rejected without invocation', () => {
  let invoked = false
  expect(() => sqlite({ get path() { invoked = true; return 'jobs.db' } })).toThrow(MqConnectionException)
