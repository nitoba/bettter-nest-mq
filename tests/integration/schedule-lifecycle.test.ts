...
import { z } from 'zod'
import { Job, MqModule, MqScheduleException, MqSchedulesService, Queue, QueueService, Schedule } from '../../src/index.ts'
...
async function appFor(fixture: ReturnType<typeof scheduleConnection>, mode: 'validate' | 'reconcile', scheduler = false) {
  return Test.createTestingModule({ imports: [
    MqModule.forRoot({ connections: { primary: fixture.connection }, schedules: { mode, sweepIntervalMs: 60_000 }, execution: { workers: false, scheduler, outboxPublisher: false } }),
...
test('changed payload drift fails validation without rewriting the operator definition', async () => {
  ...
  try {
    await deploy.init()
    await deploy.get(MqSchedulesService).upsert(LifecycleQueue, 'task', { key: 'regular', everyMs: 60_000, payload: { value: 'operator' } })
...
test('all static payloads validate before any schedule or store is acquired', async () => {
  ...
  const fixture = scheduleConnection()
  const app = await Test.createTestingModule({ imports: [MqModule.forRoot({ connections: { primary: fixture.connection }, schedules: { mode: 'reconcile' } }), MqModule.forFeature([InvalidQueue])] }).compile()
...
test('closing a scheduler waits for its admitted upstream tick before releasing the store', async () => {
  ...
  const seeded = await fixture.schedules.upsertSchedule({ ...record.value, nextRunAtMs: Date.now() - 1 })
  ...
  Object.defineProperty(fixture.schedules, 'tickSchedule', { value: async (command: TickScheduleCommand) => {
    entered.resolve()
    await gate.promise
  ...
  } finally { gate.resolve(); await worker.close() }
})
