...
import { z } from 'zod'
import { Job, MqModule, MqQueueControlsService, Queue, QueueControls, QueueControlsException, QueueService } from '../../src/index.ts'
...
function connection(store: ReturnType<typeof MemoryJobStore.make>) {
  return defineConnection({ adapter: 'memory', ownership: 'borrowed', boundary: store, scope: 'safety-test' }, (token) => ({ layer: Layer.succeed(token, store) }))
}
...
test('raw memory capabilities are not promoted to distributed guarantees by production code', async () => {
  ...
  const app = await Test.createTestingModule({ imports: [MqModule.forRoot({ connections: { first: connection(store) }, controls: { mode: 'reconcile' } }), MqModule.forFeature([FirstQueue])] }).compile()
...
test('all adapters are checked before any policy write, not one write at a time', async () => {
  ...
  const app = await Test.createTestingModule({ imports: [MqModule.forRoot({
    connections: { first: connection(first), second: connection(second) }, controls: { mode: 'reconcile' }
  }), MqModule.forFeature([FirstQueue, SecondQueue])] }).compile()
  await assert.rejects(app.init(), phase('capability'))
...
test('matching groups with different limits fail validation and only explicit reconciliation changes revision', async () => {
  ...
  const seeded = await store.reconcile(EngineControls.registry({ group: 'nestjs/queue-controls', controls: [EngineControls.define(EngineQueue.define('first'), { globalConcurrency: 7 })] }), { removal: 'ignore' })
  if (Result.isError(seeded)) throw seeded.error
  const readOnly = await Test.createTestingModule({ imports: [MqModule.forRoot({ connections: { first: connection(store) } }), MqModule.forFeature([FirstQueue])] }).compile()
  await assert.rejects(readOnly.init(), phase('drift'))
  await assert.rejects(readOnly.close(), QueueControlsException)
  const deploy = await Test.createTestingModule({ imports: [MqModule.forRoot({ connections: { first: connection(store) }, controls: { mode: 'reconcile' } }), MqModule.forFeature([FirstQueue])] }).compile()
  try {
    await deploy.init()
    expect(await deploy.get(MqQueueControlsService).get(FirstQueue)).toMatchObject({ globalConcurrency: 2, revision: 2 })
  } finally { await deploy.close() }
})

test('omitting a policy from a deployment never disables a previously persisted queue', async () => {
  const store = controlReferenceStore()
  const seeded = await store.reconcile(EngineControls.registry({ group: 'nestjs/queue-controls', controls: [EngineControls.define(EngineQueue.define('unlisted'), { globalConcurrency: 1 })] }), { removal: 'ignore' })
  if (Result.isError(seeded)) throw seeded.error
  const deploy = await Test.createTestingModule({ imports: [MqModule.forRoot({ connections: { first: connection(store) }, controls: { mode: 'reconcile' } }), MqModule.forFeature([FirstQueue])] }).compile()
  try {
    await deploy.init()
