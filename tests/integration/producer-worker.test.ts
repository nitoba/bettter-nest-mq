...
async function application(workers = true) {
  ...
  const app = await Test.createTestingModule({
    imports: [
      ...
      MqModule.forFeature([ExecutionQueue])
    ],
...
describe('real typed jobs and the upstream supervisor', () => {
  test('executes a decorated Nest service, preserves context and exposes no lease token', async () => {
    ...
    try {
      ...
      expect(app.get(MqWorkersService).workers()).toMatchObject([
        { name: 'execution-worker', state: 'running', activeCount: 0 }
      ])
    } finally {
      await app.close()
    }
  ...
  test('round-trips decoded Dates and distinguishes input from enqueueDecoded', async () => {
    ...
    try {
      ...
      expect(await queue.codec.awaitResult(first, wait)).toEqual({
        timestamp: new Date('2026-09-10T12:00:01.000Z')
      })
      expect(await queue.codec.awaitResult(second, wait)).toEqual({
        timestamp: new Date('2026-09-10T12:00:01.000Z')
      })
  ...
  test('idempotency returns one stable ID under concurrent publication', async () => {
    ...
    try {
      const ids = await Promise.all(
        Array.from({ length: 10 }, () => queue.unique.enqueue({ key: 'stable' }))
      )
  ...
  test('batch publication validates every item before any write', async () => {
    ...
    try {
      ...
      const ids = await queue.echo.enqueueMany([
        { payload: { value: 'one' } },
        { payload: { value: 'two' } }
      ])
  ...
  test('bounded waiting and explicit abort never cancel the stored job', async () => {
    ...
    try {
      ...
      await assert.rejects(
        queue.echo.awaitResult(id, { timeoutMs: 20, pollIntervalMs: 5 }),
        JobWaitTimeoutException
      )
