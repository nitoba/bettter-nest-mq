...
import { z } from 'zod'
import { Job, JobData, MqModule, MqSchedulesService, MqScheduleException, Process, Queue, QueueService, Schedule, Worker, type MqModuleOptions } from '../../src/index.ts'
...
class SchedulesQueue extends QueueService {
  ...
  @Schedule({ key: 'regular', everyMs: 60_000, payload: { text: 'scheduled' }, misfire: { strategy: 'run-once' } })
...
class SchedulesWorker {
  ...
  task(@JobData() payload: { text: string }) { return payload.text.toUpperCase() }
}
async function application(fixture: ReturnType<typeof scheduleConnection>, overrides: Partial<MqModuleOptions> = {}) {
  return Test.createTestingModule({ imports: [MqModule.forRoot({
    connections: { primary: fixture.connection }, schedules: { mode: 'reconcile', sweepIntervalMs: 60_000 }, execution: { scheduler: false }, ...overrides
  }), MqModule.forFeature([SchedulesQueue])], providers: [SchedulesWorker] }).compile()
}
...
test('reconciliation preserves pause, revision and next occurrence without disabling omitted schedules', async () => {
  ...
  try {
    ...
    await service.upsert(SchedulesQueue, 'other', { key: 'dynamic', everyMs: 90_000, payload: 'keep' })
    await service.reconcile()
    expect(await service.get(SchedulesQueue, 'task', 'regular')).toMatchObject({ paused: true, revision, nextRunAtMs })
...
test('admin operations are contract-scoped, schema-validated and do not reassign an existing address', async () => {
  ...
  try {
    await app.init()
    const service = app.get(MqSchedulesService)
    await assert.rejects(service.upsert(SchedulesQueue, 'task', { key: 'invalid', everyMs: 10, payload: { text: 1 } }), Error)
    ...
    await assert.rejects(service.upsert(SchedulesQueue, 'other', { key: 'regular', everyMs: 10, payload: 'wrong owner' }), MqScheduleException)
...
test('upstream sweeps enqueue real jobs, and a second sweep cannot fire the same occurrence', async () => {
  const fixture = scheduleConnection()
  const app = await application(fixture, { execution: { scheduler: true }, schedules: { mode: 'reconcile', sweepIntervalMs: 60_000 } })
  try {
    await app.init()
    const service = app.get(MqSchedulesService)
    ...
    const set = await fixture.schedules.upsertSchedule({ ...record.value, nextRunAtMs: Date.now() - 1 })
    if (Result.isError(set)) throw set.error
    await service.sweep()
    const fired = await service.get(SchedulesQueue, 'task', 'regular')
    assert.ok(fired?.lastJobId)
    expect(await app.get(SchedulesQueue).task.awaitResult(fired.lastJobId, { timeoutMs: 2_000, pollIntervalMs: 5 })).toBe('SCHEDULED')
    await service.sweep()
