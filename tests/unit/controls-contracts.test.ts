...
import { z } from 'zod'
import { ContractDefinitionException, getQueueDefinition, Job, MqConfiguration, Queue, QueueControls, QueueService } from '../../src/index.ts'
import { zodCodec } from '../../src/integrations/zod.ts'

const limits = { globalConcurrency: 4, perKeyConcurrency: 1, rateLimit: { max: 10, durationMs: 1_000 } }
@Queue({ name: 'controlled' })
@QueueControls(limits)
class ControlledQueue extends QueueService {
  @Job({ name: 'task', version: 1 })
  readonly task = this.job({ payload: z.object({ tenant: z.string() }), result: z.string(), dispatchKey: (payload) => payload.tenant })
}
...
test('queue control metadata is copied and deeply immutable without mutating caller configuration', () => {
  ...
  expect(definition.controls).toEqual({ globalConcurrency: 4, perKeyConcurrency: 1, rateLimit: { max: 10, durationMs: 1_000 } })
...
test.each([0, -1, 0.5, Number.NaN, Number.POSITIVE_INFINITY, Number.MAX_SAFE_INTEGER + 1])('rejects invalid positive limits %p', (value) => {
  ...
  expect(() => QueueControls({ rateLimit: { max: value, durationMs: 10 } })).toThrow(ContractDefinitionException)
  expect(() => QueueControls({ rateLimit: { max: 1, durationMs: value } })).toThrow(ContractDefinitionException)
})
...
test.each(['', ' ', '__none__', ' leading', 'trailing ', 'nul\0key', 'x'.repeat(513)])('rejects invalid or reserved derived dispatch keys %p', (key) => {
...
test('the dispatch-key callback receives decoded codec output, not the input representation', async () => {
  ...
  class Dates extends QueueService {
    readonly task = this.job({ payload: zodCodec(timestamp), result: z.string(), dispatchKey: (date) => date.toISOString() })
  }
  const job = new Dates().task
  expect(job.getDispatchKey(await job.parsePayload('2026-09-10T12:00:00.000Z'))).toBe('2026-09-10T12:00:00.000Z')
})

test('normal replicas default to read-only policy validation', () => {
  expect(new MqConfiguration({}).options.controls).toEqual({ mode: 'validate', group: 'nestjs/queue-controls' })
  expect(new MqConfiguration({ controls: { mode: 'reconcile', group: 'deploy' } }).options.controls).toEqual({ mode: 'reconcile', group: 'deploy' })
  assert.throws(() => new MqConfiguration({ controls: { group: 'x'.repeat(129) } }), ContractDefinitionException)
})
