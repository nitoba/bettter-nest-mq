...
import { makeOutboxId, makeOutboxRecord, validateOutboxRecord, MemoryOutboxStore } from 'better-effect-mq-outbox'
...
import { retryOutboxRecord, OutboxRetryAdapter, bindOutboxRetry, closeOutboxRetry, outboxRetryAdapter } from '../../src/engine/outbox-retry.ts'
...
function failed() {
  const pending = valueOf(makeOutboxRecord({
    id: valueOf(makeOutboxId('publication')),
    target: 'target', attemptsMax: 2, nowMs: 100,
    request: valueOf(makePreparedEnqueue({
      protocolVersion: 1, identity: { queue: 'reports', name: 'generate', version: 1 },
      id: 'original-job', dispatchKey: 'tenant', payload: { text: '123' }, metadata: {},
      priority: 0, attemptsMax: 4, runAt: 100, now: 100
    }))
  }))
  return valueOf(validateOutboxRecord({
    ...pending, state: 'failed', attemptsMade: 2, updatedAtMs: 200,
    failure: { kind: 'target-missing', message: 'Route was absent', retryable: false, recordedAtMs: 200 }
  }))
}
...
test('a permanent early failure receives the requested remaining budget, not its unused previous budget', () => {
  ...
  const next = retryOutboxRecord(before, { expected: { updatedAtMs: 200, attemptsMade: 1, attemptsMax: 10 }, attempts: 2 }, 300)
  expect(next.attemptsMax).toBe(3)
  expect(next.attemptsMade).toBe(1)
})

test.each(['pending', 'active', 'published'] as const)('retry never resets a %s publication', (state) => {
  expect(() => retryOutboxRecord({ ...failed(), state }, options(), 300)).toThrow(MqOutboxException)
})

test.each(['updatedAtMs', 'attemptsMade', 'attemptsMax'] as const)('each expected %s value fences stale requests', (field) => {
  const selected = options()
  expect(() => retryOutboxRecord(failed(), { ...selected, expected: { ...selected.expected, [field]: selected.expected[field] + 1 } }, 300)).toThrow(MqOutboxException)
})

test.each([0, -1, 0.5, Number.NaN, Number.POSITIVE_INFINITY])('invalid retry budget %p rejects before storage', (attempts) => {
...
test('retry rejects unsafe sums and malformed failed leases instead of repairing records', () => {
  expect(() => retryOutboxRecord(failed(), { ...options(), attempts: Number.MAX_SAFE_INTEGER }, 300)).toThrow(MqOutboxException)
  const record = { ...failed(), updatedAtMs: Number.MAX_SAFE_INTEGER }
  expect(() => retryOutboxRecord(record, { ...options(), expected: { ...options().expected, updatedAtMs: record.updatedAtMs } }, 300)).toThrow(MqOutboxException)
...
test('options are copied and accessors/unknown ownership fields fail without being invoked', () => {
  ...
  let invoked = false
  expect(() => copyOutboxRetryOptions({ get expected() { invoked = true; return input.expected }, attempts: 1 })).toThrow(MqOutboxException)
  expect(invoked).toBe(false)
  expect(() => copyOutboxRetryOptions(JSON.parse('{"attempts":1,"expected":{"updatedAtMs":1,"attemptsMade":1,"attemptsMax":1},"force":true}'))).toThrow(MqOutboxException)
})
...
test('adapter closure rejects new writes and drains already admitted failures without hiding them', async () => {
  ...
  let calls = 0
  const adapter = new OutboxRetryAdapter(async () => { calls += 1; await gate.promise; throw failure })
  const before = failed()
