...
class Reports extends QueueService {
  ...
  @Retry({ attempts: 4, backoff: { type: 'exponential', initialDelayMs: 100, factor: 2, maxDelayMs: 5_000, jitter: 0.2 } })
  @JobTimeout(1_000)
...
describe('queue metadata and identities', () => {
  test('compiles inert, immutable definitions with module/queue/job/decorator precedence', () => {
    const definition = getQueueDefinition(new Reports(), { priority: 1, timeoutMs: 9_000, delayMs: 20 })
    ...
    expect(definition.jobs[0]?.identity).toEqual({
      connection: 'primary', queue: 'reports', name: 'generate', version: 1,
      key: JSON.stringify(['primary', 'reports', 'generate', 1])
    })
    expect(definition.jobs[0]?.policy).toMatchObject({ priority: 4, timeoutMs: 1_000, delayMs: 10, retry: { attempts: 4 } })
  ...
  test('uses a stable key independent of class and property names', () => {
    ...
    expect(getQueueDefinition(new Renamed()).jobs[0]?.identity.key).toBe(getQueueDefinition(new Reports()).jobs[0]?.identity.key)
  })
  ...
  test('rejects accessors without invoking the getter', () => {
    ...
    let called = false
    Object.defineProperty(instance, 'generate', { get: () => { called = true; throw new Error('getter ran') } })
  ...
  test('copies policy configuration instead of retaining caller-owned objects', () => {
    ...
    expect(getQueueDefinition(new Copy()).jobs[0]?.policy.retry).toEqual({ attempts: 3, backoff: { type: 'fixed', delayMs: 100 } })
    expect(Object.isFrozen(retry)).toBe(false)
  })

  test.each([0, -1, 1.5, Number.NaN, Number.MAX_SAFE_INTEGER + 1])('rejects invalid job version %p', (version) => {
...
describe('policy resolution', () => {
  ...
  test('preserves explicit zero values', () => {
    expect(resolveJobPolicy({ priority: 5, delayMs: 10 }, { priority: 0, delayMs: 0 })).toMatchObject({ priority: 0, delayMs: 0 })
  })

  test.each([0, -1, 0.5, Number.POSITIVE_INFINITY])('rejects invalid attempt counts %p', (attempts) => {
  ...
  test.each([-0.1, 1.1, Number.NaN])('rejects invalid jitter %p', (jitter) => {
    expect(() => Retry({ attempts: 2, backoff: { type: 'exponential', initialDelayMs: 100, factor: 2, jitter } })).toThrow(ContractDefinitionException)
  })

  test('supports linear and named/versioned custom policy declarations', () => {
    expect(resolveJobPolicy({ retry: { attempts: 3, backoff: { type: 'linear', initialDelayMs: 10, incrementMs: 5 } } }).retry.attempts).toBe(3)
    expect(resolveJobPolicy({ retry: { attempts: 3, backoff: { type: 'custom', policy: 'provider-retry', version: 1 } } }).retry.backoff).toEqual({ type: 'custom', policy: 'provider-retry', version: 1 })
  })
})
