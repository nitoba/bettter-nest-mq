...
import { Queue as EngineQueue, QueueControls as EngineControls, makeJobName, makeQueueName, makeWorkerId, LeaseLostError } from 'better-effect-mq'
...
async function racedJob() {
  ...
  const registry = EngineControls.registry({ group: 'clock-race-test', controls: [EngineControls.define(EngineQueue.define('clock-race'), { globalConcurrency: 1 })] })
  ...
  const claimed = valueOf(await raw.claimControlled({ queue, accepted: [identity], workerId, limit: 1, leaseDurationMs: 60_000, now: now + 1, controlsRevision: revision }))
  const job = claimed.jobs[0]
  assert.ok(job)
  // Deterministically reproduce a heartbeat committing after a mutation sampled its clock.
  const renewed = valueOf(await raw.heartbeat({ jobs: [{ jobId: job.id, leaseToken: job.leaseToken }], workerId, leaseDurationMs: 60_000, now: now + 100 }))
...
test('a heartbeat that wins before settlement does not force a completed handler to run again', async () => {
  ...
  const result = valueOf(await view.settle({ jobId: job.id, leaseToken: job.leaseToken, startedAt: now + 1, now: now + 10, outcome: { type: 'complete', result: { value: 'done' } } }))
  expect(result.status).toBe('applied')
  const stored = valueOf(await raw.getJob({ jobId: job.id }))
  expect(stored).toMatchObject({ state: 'completed', deliveryCount: 1, attemptsMade: 1, result: { value: 'done' } })
...
test('retry settlement preserves its declared delay when a rejected clock sample is refreshed', async () => {
  const { raw, view, job, now } = await racedJob()
  valueOf(await view.settle({ jobId: job.id, leaseToken: job.leaseToken, startedAt: now + 1, now: now + 10, outcome: {
    type: 'retry', runAt: now + 510, retryDelayMs: 500,
...
test('clock recovery never makes an expired lease eligible to settle again', async () => {
  ...
  const result = await view.settle({ jobId: job.id, leaseToken: job.leaseToken, now: now + 100_000, outcome: { type: 'complete', result: { value: 'late' } } })
