...
import { z } from 'zod'
import { Job, Queue, QueueControls, QueueService, Schedule, MqConfiguration, MqScheduleException, getQueueDefinition } from '../../src/index.ts'
...
test('repeatable schedule declarations copy JSON input without freezing the caller', async () => {
  ...
  expect(definitions.find((entry) => entry.schedule.key === 'daily')?.encoded).toEqual({ label: 'original' })
  expect(Object.isFrozen(payload)).toBe(false)
  expect(scheduleMetadata(queue, 'task')).toHaveLength(2)
})

test.each(['', 'bad', '* * *', '* * * * * *', '99 * * * *'])('rejects invalid five-field cron %p', (cron) => {
  expect(() => Schedule({ key: 'bad', cron, payload: null })).toThrow(MqScheduleException)
})
test.each([0, -1, 0.5, Number.NaN, Number.POSITIVE_INFINITY])('rejects invalid interval %p', (everyMs) => {
...
test('rejects invalid timezone, empty keys and invalid missed-occurrence policies', () => {
  expect(() => Schedule({ key: 'bad', cron: '0 0 * * *', timeZone: 'not-a-zone', payload: null })).toThrow(MqScheduleException)
  expect(() => Schedule({ key: '', everyMs: 10, payload: null })).toThrow(MqScheduleException)
  expect(() => Schedule({ key: 'bad', everyMs: 10, payload: null, misfire: { strategy: 'catch-up', maxOccurrences: 0 } })).toThrow(MqScheduleException)
})
test('duplicate addresses are rejected across different job properties before persistence', async () => {
  ...
  await assert.rejects(compileSchedules(getQueueDefinition(new Duplicate()).jobs), MqScheduleException)
})
...
test('schedule payloads are checked through explicit codecs before encoding', async () => {
  const timestamp = z.codec(z.iso.datetime(), z.date(), { decode: (value) => new Date(value), encode: (date) => date.toISOString() })
  @Queue({ name: 'dates' })
...
test('per-key queues fail clearly because the pinned schedule protocol has no dispatch-key field', async () => {
  ...
  class Keyed extends QueueService {
    ...
    readonly task = this.job({ payload: z.string(), result: z.string(), dispatchKey: (value) => value })
  }
