...
async function acquiredJob(now = Date.now()) {
  ...
  const identity = {
    queue: valueOf(makeQueueName('clock-safety')),
  ...
  const registry = EngineControls.registry({
    group: 'clock-safety',
    controls: [EngineControls.define(EngineQueue.define('clock-safety'), { globalConcurrency: 1 })]
  })
  ...
  return {
    raw,
    view: dispatchStore(raw, [getQueueDefinition(new SafetyQueue())]),
    job,
    now,
    claim,
    revision
...
test('a refreshed clock never grants an old token the replacement delivery lease', async () => {
  ...
  const result = await view.settle({
    ...
    leaseToken: job.leaseToken,
    now: now + 10,
    outcome: { type: 'complete', result: { value: 'obsolete' } }
  })
...
test('duplicate completion acknowledgment never adds a second attempt after clock refresh', async () => {
  ...
  const request = {
    ...
    leaseToken: job.leaseToken,
    now: now + 10,
    outcome: { type: 'complete' as const, result: { value: 'done' } }
  }
...
test('refresh is bounded even if every attempted mutation loses to another heartbeat', async () => {
  ...
  const result = await withFreshControlledClock(raw, job.id, now + 10, async (sample) => {
    ...
    return raw.settleControlled({
      jobId: job.id,
      leaseToken: job.leaseToken,
      now: sample,
      controlsRevision: revision,
      outcome: { type: 'complete', result: { value: 'never applied' } }
    })
  })
  assert.ok(Result.isError(result))
  expect(result.error).toBeInstanceOf(JobDefinitionError)
  expect(calls).toBe(4)
  expect(valueOf(await raw.getJob({ jobId: job.id }))).toMatchObject({
    state: 'active',
    deliveryCount: 1,
    attemptsMade: 0
  })
...
test('ambiguous infrastructure errors and unrelated validation errors are never replayed', async () => {
  ...
  for (const error of [
    ...
    let calls = 0
    const result = await withFreshControlledClock(raw, job.id, now + 10, () => {
      calls += 1
