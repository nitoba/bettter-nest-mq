...
const prepared: PreparedJob = {
  ...
  request: {
    protocolVersion: 1,
    identity: { queue: 'outbox-test', name: 'echo', version: 1 },
    payload: '123',
    metadata: {},
    priority: 0,
    runAt: 1_000,
...
test('explicit job ids and publication budgets remain independent of job execution policy', async () => {
  const record = await compileOutboxRecord(
    'source',
    {
      id: 'record',
      job: { ...prepared, request: { ...prepared.request, id: 'explicit-job' } },
      attempts: 3
    },
    jobs
  )
...
test('unregistered target identities and invalid wire payloads are rejected before a transaction', async () => {
  await assert.rejects(
    compileOutboxRecord('source', { id: 'id', job: { ...prepared, connection: 'missing' } }, jobs),
    MqOutboxException
  )
  await assert.rejects(
    compileOutboxRecord(
      'source',
      { id: 'id', job: { ...prepared, request: { ...prepared.request, payload: 1 } } },
      jobs
    )
  )
})
...
test('duplicate comparison includes route and dispatchKey, not just the upstream digest', async () => {
  ...
  expect(() => assertSameOutboxContent(record, { ...record, target: 'other' })).toThrow(
    MqOutboxException
  )
  expect(() =>
    assertSameOutboxContent(record, {
      ...record,
      request: { ...record.request, dispatchKey: 'other' }
    })
  ).toThrow(MqOutboxException)
  expect(() => assertSameOutboxContent(record, { ...record, attemptsMax: 20 })).toThrow(
    MqOutboxException
  )
})

test.each([0, -1, 1.5, Number.NaN, Number.POSITIVE_INFINITY])(
  'rejects invalid publication attempt counts %p',
  async (attempts) => {
    await assert.rejects(
      compileOutboxRecord('source', { id: 'record', job: prepared, attempts }, jobs),
      MqOutboxException
    )
  }
)
...
test('publisher configuration is copied, validates timing and has independent role enablement', () => {
  ...
  expect(
    () => new MqConfiguration({ outbox: { leaseDurationMs: 100, heartbeatIntervalMs: 100 } })
  ).toThrow(MqOutboxException)
  expect(
    () => new MqConfiguration({ outbox: { retryBaseDelayMs: 100, retryMaxDelayMs: 10 } })
  ).toThrow(MqOutboxException)
})

test.each([0, -1, 0.5, Number.NaN, 2_147_483_648])(
  'rejects unsafe publisher timer values %p',
  (pollIntervalMs) => {
