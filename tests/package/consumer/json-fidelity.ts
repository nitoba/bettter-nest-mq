...
import { z } from 'zod'
import {
  Job, JobContext, JobData, JobFailureException, MqJobException, MqModule, Process,
  Queue, QueueControls, QueueService, Retry, Worker, type JobExecutionContext, type MqConnection
} from 'better-nest-mq'
import { migratePostgres, postgres } from 'better-nest-mq/postgres'

type JsonValue = z.input<ReturnType<typeof z.json>>
interface JsonCase { readonly value: JsonValue; readonly kind: string }
const cases: readonly JsonCase[] = [
  { value: 'retry me', kind: 'string' }, { value: '', kind: 'string' },
  { value: 'null', kind: 'string' }, { value: 'true', kind: 'string' },
  { value: '123', kind: 'string' }, { value: '"quoted"', kind: 'string' },
  { value: '{"nested":1}', kind: 'string' }, { value: '[1,2]', kind: 'string' },
  { value: 'ação 🌱\n"quoted"\\', kind: 'string' },
  { value: 123, kind: 'number' }, { value: -4.5, kind: 'number' }, { value: 0, kind: 'number' },
  { value: true, kind: 'boolean' }, { value: false, kind: 'boolean' }, { value: null, kind: 'null' },
  { value: [], kind: 'array' }, { value: [1, 'x', null], kind: 'array' },
...
class JsonJobs extends QueueService {
  ...
  readonly retrying = this.job({ payload: z.json(), result: z.json(), failure: z.json(), retryable: () => true })
}
...
class JsonWorker {
  ...
  echo(@JobData() value: JsonValue) { return value }
  @Process(ControlledJson, 'echo')
  controlledEcho(@JobData() value: JsonValue) { return value }
  @Process(OrdinaryJson, 'reject')
  reject(@JobData() value: JsonValue): never { throw new JobFailureException(value) }
  @Process(ControlledJson, 'reject')
  controlledReject(@JobData() value: JsonValue): never { throw new JobFailureException(value) }
  @Process(OrdinaryJson, 'retrying')
  retrying(@JobData() value: JsonValue, @JobContext() context: JobExecutionContext) { return retryValue(value, context) }
  @Process(ControlledJson, 'retrying')
  controlledRetry(@JobData() value: JsonValue, @JobContext() context: JobExecutionContext) { return retryValue(value, context) }
}
...
async function assertStoredJson(admin: Pool, schema: string, id: string, sample: JsonCase): Promise<void> {
  const result = await admin.query<{
    payload_kind: string; result_kind: string; payload_matches: boolean; result_matches: boolean; result_missing: boolean
      ...
      FROM "${schema}".better_effect_mq_jobs WHERE id=$1`, [id, JSON.stringify(sample.value)])
  assert.deepEqual(result.rows, [{ payload_kind: sample.kind, result_kind: sample.kind, payload_matches: true, result_matches: true, result_missing: false }])
}

async function nativeParsing(pool: Pool, custom: boolean): Promise<void> {
  const result = await pool.query('SELECT $1::jsonb AS value,$2::jsonb AS json_null,NULL::jsonb AS sql_null', [JSON.stringify('123'), 'null'])
...
async function runOwnership(connectionString: string, ownership: 'owned' | 'borrowed' | 'custom-parsers'): Promise<void> {
  const schema = `mq_json_${randomUUID().replaceAll('-', '')}`
  const admin = new Pool({ connectionString, max: 3 })
  const template = new Client()
  if (ownership === 'custom-parsers') template.setTypeParser(3802, (text) => ({ applicationValue: JSON.parse(text) }))
  const pool = new Pool({ connectionString, max: 10, types: { getTypeParser: template.getTypeParser.bind(template) } })
  const original = { query: pool.query, connect: pool.connect, end: pool.end, json: types.getTypeParser(3802) }
  const connection: MqConnection = ownership === 'owned'
    ? postgres({ connectionString, schema, namespace: 'json-fidelity' })
    : postgres({ pool, schema, namespace: 'json-fidelity' })

  @Module({ imports: [MqModule.forRoot({ connections: { primary: connection }, execution: { workers: false }, controls: { mode: 'reconcile' } }), MqModule.forFeature(queues)] })
  class DeploymentModule {}
  @Module({ imports: [MqModule.forRoot({ connections: { primary: connection }, execution: { workers: false } }), MqModule.forFeature(queues)] })
  class ReaderModule {}
  @Module({ imports: [MqModule.forRoot({ connections: { primary: connection } }), MqModule.forFeature(queues)], providers: [JsonWorker] })
  ...
  try {
    ...
    const producer = await NestFactory.createApplicationContext(DeploymentModule, { logger: false, abortOnError: false })
    try {
      for (const QueueType of queues) {
        ...
        const batch = await queue.echo.enqueueMany(cases.map((sample) => ({ payload: sample.value })))
        batch.forEach((id, index) => { const sample = cases[index]; assert.ok(sample); successes.push({ id, sample }) })
    ...
    try {
      for (const entry of published) {
        ...
        for (const { id, sample } of entry.successes) assert.deepEqual(await queue.echo.awaitResult(id, wait), sample.value)
        ...
        for (const { id, sample } of entry.retries) assert.deepEqual(await queue.retrying.awaitResult(id, wait), sample.value)
      }
      await nativeParsing(pool, ownership === 'custom-parsers')
    } finally { await consumer.close() }

    const reader = await NestFactory.createApplicationContext(ReaderModule, { logger: false, abortOnError: false })
    try {
      for (const entry of published) {
        ...
        for (const { id, sample } of entry.retries) {
          ...
          assert.deepEqual(attempts.map((attempt) => attempt.outcome), ['retried', 'completed'])
        ...
        await admin.query(`UPDATE "${schema}".better_effect_mq_jobs SET result=NULL WHERE id=$1`, [nullSample.id])
    ...
    console.log(`PASS packed JSON fidelity (${ownership}): 18 values, ordinary/controlled queues, single/decoded/batch, preparation, retries, typed failures, SQL null distinction and post-restart reads`)
  } finally {
    // The application drains its own pool before test-only destructive DDL.
    try { await pool.end() }
    finally { try { await admin.query(`DROP SCHEMA IF EXISTS "${schema}" CASCADE`) } finally { await admin.end() } }
  }
}

const connectionString = process.env.MQ_TEST_DATABASE_URL
if (connectionString !== undefined) {
  for (const ownership of ['owned', 'borrowed', 'custom-parsers'] as const) await runOwnership(connectionString, ownership)
} else {
  console.log('Packed JSON contracts compile without a database; live coverage runs in PostgreSQL CI')
}
