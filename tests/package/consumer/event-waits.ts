...
import { z } from 'zod'
import {
  Job, JobCancelledException, JobData, JobFailureException, JobWaitAbortedException,
  JobWaitTimeoutException, MqJobException, MqModule, Process, Queue, QueueService,
  Worker, type JobWaitOptions, type PayloadOf
} from 'better-nest-mq'
import { migratePostgres, postgres } from 'better-nest-mq/postgres'
import { zodCodec } from 'better-nest-mq/zod'

const Json = z.union([z.string(), z.number(), z.boolean(), z.null(), z.array(z.string()), z.object({ value: z.string() })])
const DateValue = zodCodec(z.codec(z.iso.datetime(), z.date(), { decode: (input) => new Date(input), encode: (date) => date.toISOString() }))
@Injectable()
...
class EventQueue extends QueueService {
  ...
  readonly failure = this.job({ payload: z.string(), result: z.string(), failure: z.object({ code: z.literal('expected') }) })
...
class EventWorker {
  ...
  echo(@JobData() payload: PayloadOf<EventQueue['echo']>) { return payload }
  ...
  date(@JobData() payload: PayloadOf<EventQueue['date']>) { assert.ok(payload instanceof Date); return payload }
}
...
async function verifyEventWaits(connectionString: string): Promise<void> {
  ...
  const inputs = ['123', 'true', '{"json":"string"}', '', 'ação', 123, false, null, ['array'], { value: 'object' }]
  const connection = postgres({ pool, schema, namespace: 'events', events: true, outbox: true, schedules: true, flows: true })
  @Module({ imports: [MqModule.forRoot({ connections: { primary: connection }, execution: { workers: false, scheduler: false, outboxPublisher: false } }), MqModule.forFeature([EventQueue])] })
  class ProducerModule {}
  @Module({ imports: [MqModule.forRoot({ connections: { primary: postgres({ connectionString, schema, namespace: 'events' }) } }), MqModule.forFeature([EventQueue])], providers: [EventWorker] })
  class WorkerModule {}
  try {
    ...
    try {
      ...
      const cancelled = assert.rejects(queue.echo.awaitResult(cancellation, wait), JobCancelledException)
      await queue.echo.cancel(cancellation)
      await cancelled
      await assert.rejects(queue.echo.awaitResult(ids[0]!, { strategy: 'events', timeoutMs: 30, pollFallbackMs: 1000 }), JobWaitTimeoutException)
      assert.equal((await queue.echo.poll(ids[0]!))?.state, 'waiting')
      const aborted = new AbortController()
      aborted.abort('caller left')
      await assert.rejects(queue.echo.awaitResult(ids[0]!, { ...wait, signal: aborted.signal }), JobWaitAbortedException)
      const pending = Promise.all(ids.map((id) => queue.echo.awaitResult(id, wait)))
      const worker = await NestFactory.createApplicationContext(WorkerModule, { logger: false, abortOnError: false })
      try {
        ...
        const activation = await pool.query<{ activation_state: string }>(`SELECT activation_state FROM "${schema}".better_effect_mq_job_event_activation`)
        assert.ok(activation.rows.length > 0)
        assert.ok(activation.rows.every((row) => row.activation_state === 'optional'), 'Reader opt-in must never promote event persistence to required')
        console.log('PASS event-log-assisted results across separate producer/worker contexts, JSON/null/Date, terminal failures, cancellation, timeout and stable namespaces')
    ...
    try {
      for (const [index, id] of ids.entries()) assert.deepEqual(await restarted.get(EventQueue).echo.awaitResult(id, wait), inputs[index])
    } finally { await restarted.close() }
    assert.equal((await pool.query<{ value: number }>('SELECT 1 AS value')).rows[0]?.value, 1)
    console.log('PASS completed results remain readable after restarting the reader and borrowed pool stays usable')
  } finally {
...
else console.log('Packed event-wait types passed; real PostgreSQL result waits run in the database CI job')
