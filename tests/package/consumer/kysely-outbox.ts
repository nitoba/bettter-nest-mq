...
import { z } from 'zod'
import { Job, JobData, MqModule, MqOutboxService, MqOutboxException, Process, Queue, QueueService, Worker, type JobJsonValue } from 'better-nest-mq'
...
class BusinessWorker {
  ...
  echo(@JobData() payload: JobJsonValue) { return payload }
}
function barrier() {
  let open = () => { throw new Error('Barrier not initialized') }
...
async function verify(databaseUrl: string): Promise<void> {
  ...
  @Module({ imports: [MqModule.forRoot({ connections: { primary: connection }, execution: { workers: false, outboxPublisher: false } }), MqModule.forFeature([BusinessQueue])] })
  class ProducerModule {}
  @Module({ imports: [MqModule.forRoot({ connections: { primary: connection }, outbox: { pollIntervalMs: 10 } }), MqModule.forFeature([BusinessQueue])], providers: [BusinessWorker] })
  ...
  try {
    ...
    const app = await NestFactory.createApplicationContext(ProducerModule, { logger: false, abortOnError: false })
    try {
      ...
      const transaction = client.transaction(async (tx) => {
        ...
        const row = await db.insertInto('business').values({ id, note: 'created', payload }).returningAll().executeTakeFirstOrThrow()
        const appended = await tx.append(entry)
        assert.equal(appended.duplicate, false)
        const identity = (await sql<{ pid: number; txid: string }>`select pg_backend_pid() as pid, txid_current()::text as txid`.execute(db)).rows[0]
        const trace = (await sql<{ pid: number; txid: string }>`select pid, txid from ${sql.table(`${schema}.append_trace`)} where id=${entry.id}`.execute(db)).rows[0]
      ...
      try {
        ...
        assert.equal((await admin.query(`SELECT id FROM "${schema}".business WHERE id=$1`, [id])).rowCount, 0)
        assert.equal(await service.get('primary', entry.id), undefined)
      } finally { release.open() }
      const saved = await transaction
      assert.equal((await admin.query(`SELECT id FROM "${schema}".business WHERE id=$1`, [id])).rowCount, 1)
      assert.equal((await service.get('primary', entry.id))?.state, 'pending')
      assert.deepEqual((await service.get('primary', entry.id))?.request.payload, payload, 'Adapter JSON stays independent of application parsers')
      ...
      console.log('PASS Kysely business query and outbox append share the exact native transaction/backend; uncommitted writes remain invisible')

      for (const mode of ['callback', 'query', 'append', 'nested'] as const) {
        const failedId = randomUUID()
        const failedEntry = { id: `failed:${failedId}`, job: await jobs.echo.prepare({ failedId }, { jobId: failedId }) }
        let calls = 0
        await assert.rejects(client.transaction(failedEntry, async (tx) => {
          calls += 1
          const db = tx.db.withSchema(schema)
          await db.insertInto('business').values({ id: failedId, note: mode, payload: null }).execute()
          if (mode === 'callback') throw new Error('Intentional business failure')
          if (mode === 'query') await assert.rejects(db.insertInto('business').values({ id: failedId, note: mode, payload: null }).execute())
          if (mode === 'append') await assert.rejects(tx.append({ ...failedEntry, id: '' }))
          if (mode === 'nested') await assert.rejects(db.transaction().execute(async () => 1), MqOutboxException)
        }))
        assert.equal(calls, 1, 'Business callbacks must not replay automatically')
        assert.equal((await admin.query(`SELECT id FROM "${schema}".business WHERE id=$1`, [failedId])).rowCount, 0)
        assert.equal(await service.get('primary', failedEntry.id), undefined)
      }
      console.log('PASS callback, caught SQL/append failure and rejected nested transaction all roll back domain and outbox')

      // Predeclared batch entries retain the existing outbox validation/append protocol.
      const batch = await Promise.all([null, '123'].map(async (value) => {
      ...
      const updated = await client.transaction(batch, async ({ db }) => {
        const result = await db.withSchema(schema).updateTable('business').set({ note: 'updated' }).where('id', '=', id).executeTakeFirst()
        assert.equal(result.numUpdatedRows, 1n)
        return result.numUpdatedRows
      })
      assert.equal(updated, 1n)
      await assert.rejects(kyselyOutbox<Database>(service, 'missing').transaction(() => 1), MqOutboxException)
      const duplicate = await client.transaction(async (tx) => (await tx.append(entry)).duplicate)
      assert.equal(duplicate, true)
    } finally { await app.close() }

    const processor = await NestFactory.createApplicationContext(ProcessorModule, { logger: false, abortOnError: false })
    try {
      for (const item of delivered) assert.deepEqual(await processor.get(BusinessQueue).echo.awaitResult(item.id, { timeoutMs: 10_000, pollIntervalMs: 10 }), item.payload)
      const deadline = Date.now() + 10_000
      while ((await processor.get(MqOutboxService).counts('primary')).published !== delivered.length) {
        assert.ok(Date.now() < deadline)
        await sleep(10)
      }
    } finally { await processor.close() }
    assert.equal((await pool.query<{ value: number }>('SELECT 1 AS value')).rows[0]?.value, 1)
    console.log('PASS Kysely batch append and scalar/null jobs publish/process after producer restart; borrowed pool remains open')
  } finally {
    try { await pool.end() }
...
else console.log('Packed Kysely types passed; transaction identity and rollback run in PostgreSQL CI')
