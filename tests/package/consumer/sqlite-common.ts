...
import { z } from 'zod'
import {
  Job, JobContext, JobData, JobCancelledException, JobFailureException, JobWaitTimeoutException,
  MqConnectionsService, MqModule, Process, Queue, QueueService, Retry, Worker,
  type MqConnection, type JobExecutionContext
} from 'better-nest-mq'
...
const values = ['123', 'true', 'null', '{"value":1}', '', 'ação', 0, 7, false, null, ['array'], { value: 'object' }]
const timestamp = zodCodec(z.codec(z.iso.datetime(), z.date(), { decode: (value) => new Date(value), encode: (date) => date.toISOString() }))
...
class Jobs extends QueueService {
  ...
  readonly retry = this.job({ payload: z.string(), result: z.string(), failure: z.object({ code: z.literal('temporary') }), retryable: () => true })
...
class Processor {
  ...
  @Process(Jobs, 'date')
  date(@JobData() value: Date) { assert.ok(value instanceof Date); return value }
}
async function app(api: SqliteApi, path: string, workers: boolean, namespace = 'native-hosts') {
  @Module({ imports: [MqModule.forRoot({ connections: { primary: api.sqlite({ path, namespace, pollIntervalMs: 5 }) }, execution: { workers } }), MqModule.forFeature([Jobs])], providers: [Processor] })
  class Application {}
...
async function consume(api: SqliteApi, path: string): Promise<void> {
  ...
  try {
    ...
    assert.deepEqual(await Promise.all(values.map((_, index) => jobs.echo.awaitResult(`json-${index}`, wait))), values)
    assert.equal(await jobs.retry.awaitResult('retry', wait), 'after retry')
    assert.deepEqual((await jobs.retry.attempts('retry')).map((attempt) => attempt.outcome), ['retried', 'completed'])
    assert.deepEqual(await jobs.date.awaitResult('date', wait), new Date('2026-09-15T12:00:00.000Z'))
...
export async function verifySqlite(api: SqliteApi, runtime: 'node' | 'bun'): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      await jobs.echo.enqueueMany(values.map((payload, index) => ({ payload, options: { jobId: `json-${index}` } })))
      ...
      assert.equal(await jobs.echo.enqueue('deduplicated', { idempotencyKey: 'stable' }), await jobs.echo.enqueue('deduplicated', { idempotencyKey: 'stable' }))
      ...
      await assert.rejects(jobs.echo.awaitResult('json-0', { timeoutMs: 10 }), JobWaitTimeoutException)
    ...
    assert.ok(script)
    const args = runtime === 'node' ? ['--experimental-sqlite', script, 'consume', path] : [script, 'consume', path]
    const child = spawn(process.execPath, args, { stdio: 'inherit' })
    const timeout = setTimeout(() => child.kill('SIGKILL'), 15_000)
    try {
      await new Promise<void>((resolve, reject) => {
        child.once('error', reject)
        child.once('exit', (code, signal) => code === 0 ? resolve() : reject(new Error(`SQLite consumer exited ${code}/${signal}`)))
      })
    } finally { clearTimeout(timeout) }
    const reader = await app(api, path, false)
    try {
      const jobs = reader.get(Jobs)
      for (const [index, value] of values.entries()) assert.deepEqual((await jobs.echo.poll(`json-${index}`))?.result, value)
    ...
    try { assert.equal(await isolated.get(Jobs).echo.poll('json-0'), undefined) }
    finally { await isolated.close() }
    console.log(`PASS ${runtime} SQLite: native file persistence, independent worker process, JSON/null/Date, retries, cancellation, promotion and namespace isolation`)
