...
const messageSchema = z.union([
  z.object({ type: z.literal('ready'), pid: z.int() }),
  z.object({ type: z.enum(['done', 'error']), id: z.string(), message: z.string().optional() })
])
class SchedulerProcess {
  ...
  constructor(connectionString: string, schema: string) {
    this.child = spawn('node', [fileURLToPath(new URL('./schedule-child.js', import.meta.url))], {
      ...
      env: { ...process.env, MQ_TEST_DATABASE_URL: connectionString, MQ_SCHEDULE_SCHEMA: schema }
    })
    this.child.stderr?.on('data', (chunk: Buffer) => {
      this.errors = `${this.errors}${chunk.toString()}`.slice(-8_000)
    })
  ...
  async sweep(): Promise<void> {
    ...
    const timer = setTimeout(() => {
      this.pending.get(id)?.reject(new Error('Scheduler IPC sweep timed out'))
      this.pending.delete(id)
    }, 10_000)
    try {
      await new Promise<void>((resolve, reject) => {
        this.pending.set(id, { resolve, reject })
        this.child.send({ id, type: 'sweep' })
      })
    } finally {
      clearTimeout(timer)
    }
...
class ScheduledWorker {
  ...
  echo(@JobData() value: PayloadOf<ScheduledQueue['echo']>) {
    return value
  }
  @Process(ScheduledQueue, 'date')
  date(@JobData() value: PayloadOf<ScheduledQueue['date']>) {
    assert.ok(value.timestamp instanceof Date)
    return value
  }
}
...
async function verify(connectionString: string): Promise<void> {
  ...
  @Module({
    imports: [
      MqModule.forRoot({
        ...
        execution: { scheduler: false, workers: false, outboxPublisher: false },
  ...
  @Module({
    ...
    providers: [ScheduledWorker]
  })
  class ConsumerModule {}
  try {
    await migratePostgres({ pool, schema })
    const app = await NestFactory.createApplicationContext(DeployModule, {
      logger: false,
      abortOnError: false
    })
    try {
      ...
      const cron = await schedules.upsert(ScheduledQueue, 'echo', {
        key: 'cron',
        cron: '0 9 * * *',
      ...
      const due = async (key: string, ago = 10) => {
        ...
        const result = await pool.query(
          `UPDATE "${schema}".better_effect_mq_schedules SET next_run_at_ms=$1 WHERE schedule_group=$2 AND schedule_key=$3`,
          [value, 'nestjs/schedules', key]
        )
      ...
      const count = async () =>
        (
          await pool.query<{ count: number }>(
            `SELECT count(*)::integer AS count FROM "${schema}".better_effect_mq_jobs`
          )
        ).rows[0]?.count ?? 0
      const inputs: z.output<typeof ValueSchema>[] = [
        'true',
        '123',
        '{"looks":"json"}',
        '',
      ...
      for (const [index, payload] of inputs.entries()) {
        ...
        assert.equal(
          await count(),
          before + 1,
          'Competing schedulers must persist one job for the same slot'
        )
        await both()
      ...
      console.log(
        'PASS independent scheduler processes, scalar/null JSON, stable occurrence fencing and cron timezone'
      )

      const beforeMisfires = await count()
      await schedules.upsert(ScheduledQueue, 'echo', {
        key: 'catch-up',
        everyMs: 60_000,
        payload: 'catch-up',
      ...
      await schedules.upsert(ScheduledQueue, 'date', {
        key: 'date',
        everyMs: 60_000,
        payload: dateInput
      })
      ...
      try {
        ...
        for (const result of completed)
          assert.deepEqual(await queue.echo.awaitResult(result.id, wait), result.value)
        assert.deepEqual(await queue.date.awaitResult(dateRecord.lastJobId, wait), {
          timestamp: new Date(dateInput.timestamp)
        })
        assert.equal(
          (await consumer.get(MqSchedulesService).get(ScheduledQueue, 'echo', 'regular'))?.revision,
          paused?.revision
        )
      } finally {
      ...
      console.log(
        'PASS pause/resume, decoded Date result, later worker processing and persisted schedule revisions'
      )
    } finally {
      await app.close()
    }
...
if (connectionString !== undefined) await verify(connectionString)
  ...
  console.log(
    'Packed schedule APIs and types passed; real competing schedulers run in PostgreSQL CI'
  )

// @ts-expect-error A schedule cannot combine relative intervals and cron.
export const invalid: ScheduleOptions = {
  key: 'invalid',
  everyMs: 10,
  cron: '* * * * *',
  payload: null
