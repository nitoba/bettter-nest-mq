  ...
  async run(payload: { key: string; gate: string }, context: JobExecutionContext) {
    ...
    try {
      while (!context.signal.aborted) {
        const result = await pool.query<{ opened: boolean }>(
          `SELECT opened FROM "${schema}".gates WHERE name=$1`,
          [payload.gate]
        )
      ...
      await pool.query(`UPDATE "${schema}".handler_probe SET finished=true WHERE job_id=$1`, [
        context.jobId
      ])
    }
  }
...
class LeftWorker {
  ...
  global(
    @JobData() payload: { key: string; gate: string },
    @JobContext() context: JobExecutionContext
  ) {
    return this.probe.run(payload, context)
  }
...
@Module({
  imports: [
    MqModule.forRoot({
      connections: {
        primary: postgres({
          connectionString: config.MQ_TEST_DATABASE_URL,
          schema,
          namespace: 'controls'
        })
      }
