...
class PackedQueue extends QueueService {
  ...
  readonly retrying = this.job({
    payload: z.object({ retryable: z.boolean() }),
    result: z.int(),
...
@Injectable()
class Labels {
  readonly prefix = 'from-real-nest-di'
}
...
class PackedWorker {
  ...
  transform(
    @JobData() payload: PayloadOf<PackedQueue['transform']>
  ): ResultOf<PackedQueue['transform']> {
    assert.ok(payload.timestamp instanceof Date)
    return {
      label: `${this.labels.prefix}:${payload.id}`,
      timestamp: new Date(payload.timestamp.getTime() + 1_000)
    }
  }

  @Process(PackedQueue, 'retrying')
  retrying(
    @JobData() payload: PayloadOf<PackedQueue['retrying']>,
    @JobContext() context: JobExecutionContext
  ) {
    if (context.attempt < 3)
      throw new JobFailureException({ code: 'unavailable', retryable: payload.retryable })
...
export async function verifyExecution(connectionString: string): Promise<void> {
  ...
  try {
    ...
    @Module({
      imports: [
        MqModule.forRoot({
          connections: { primary: postgres(options) },
    ...
    try {
      ...
      await assert.rejects(
        queue.transform.awaitResult(first, { timeoutMs: 25, pollIntervalMs: 5 }),
        JobWaitTimeoutException
      )
      assert.equal((await queue.transform.poll(first))?.state, 'waiting')
      const prepared = await queue.transform.prepare(
        { ...input, id: 'prepared' },
        { jobId: 'not-published' }
      )
      assert.equal(prepared.connection, 'primary')
      assert.equal(await queue.transform.poll('not-published'), undefined)
      const delayed = await queue.transform.enqueue(
        { ...input, id: 'cancel-before-work' },
        { delayMs: 60_000 }
      )
    ...
    try {
      ...
      assert.deepEqual(await queue.transform.awaitResult(savedId, wait), {
        label: 'from-real-nest-di:survives-producer-exit',
        timestamp: new Date('2026-09-10T12:00:01.000Z')
      })
      ...
      assert.deepEqual(
        (await queue.retrying.attempts(retryId)).map((attempt) => attempt.outcome),
      ...
      assert.deepEqual(
        await queue.transform.execute(
          { id: 'execute', timestamp: '2026-09-10T12:00:00.000Z' },
          { wait }
        ),
        {
          label: 'from-real-nest-di:execute',
          timestamp: new Date('2026-09-10T12:00:01.000Z')
        }
      )
    ...
    const reader = await NestFactory.createApplicationContext(ProducerModule, {
      logger: false,
      abortOnError: false
    })
    try {
    ...
    console.log(
      'PASS packed real PostgreSQL execution: producer restart, workers, DI, codecs, idempotency, retries, terminal failures and durable results'
    )
  } finally {
    try {
...
if (connectionString !== undefined) await verifyExecution(connectionString)
  ...
  console.log(
    'Packed worker declarations and types passed; live execution runs in the PostgreSQL CI job'
  )
