...
import { Pool } from 'pg'
import { z } from 'zod'
import { JobCancelledException, MqModule, MqQueueControlsService, QueueControlsException } from 'better-nest-mq'
...
class WorkerProcess {
  ...
  constructor(connectionString: string, schema: string, role: 'left' | 'right') {
    this.child = spawn('node', [fileURLToPath(new URL('./controls-worker.js', import.meta.url))], {
      ...
      env: { ...process.env, MQ_TEST_DATABASE_URL: connectionString, MQ_CONTROL_SCHEMA: schema, MQ_CONTROL_ROLE: role }
    })
    this.child.stderr?.on('data', (chunk: Buffer) => { this.errors = `${this.errors}${chunk.toString()}`.slice(-16_000) })
    this.exited = new Promise((resolve) => { this.child.once('exit', resolve) })
    this.ready = new Promise((resolve, reject) => {
      this.child.once('error', reject)
      this.child.once('exit', (code) => reject(new Error(`Worker exited before readiness: ${code}: ${this.errors}`)))
      this.child.on('message', (message) => { const parsed = Ready.safeParse(message); if (parsed.success) resolve(parsed.data.pid) })
    })
  }
...
async function verifyDistributed(connectionString: string): Promise<void> {
  ...
  try {
    ...
    @Module({ imports: [MqModule.forRoot({ connections: { primary: connection }, execution: { workers: false }, controls: { mode: 'reconcile' } }), MqModule.forFeature(ControlQueues)] })
    class DeployModule {}
    @Module({ imports: [MqModule.forRoot({ connections: { primary: connection }, execution: { workers: false } }), MqModule.forFeature(ControlQueues)] })
    class ProducerModule {}
    const deploy = await NestFactory.createApplicationContext(DeployModule, { logger: false, abortOnError: false })
    let revision: number
    try {
    ...
    try {
      ...
      const readyTimeout = setTimeout(() => { left.child.kill('SIGKILL'); right.child.kill('SIGKILL') }, 10_000)
      let pids: number[]
      ...
      const entered = async (id: string): Promise<boolean> => (await admin.query(`SELECT 1 FROM "${schema}".handler_probe WHERE job_id=$1`, [id])).rowCount === 1
      ...
      const gRight = await global.right.enqueueMany(Array.from({ length: 5 }, (_, index) => ({ payload: { key: `right-${index}`, gate: 'global' } })))
      await until(async () => (await admin.query(`SELECT 1 FROM "${schema}".handler_probe WHERE queue='distributed-global' AND NOT finished`)).rowCount === 2, 'global capacity reached across processes')
      const activePids = await admin.query<{ pid: number }>(`SELECT DISTINCT pid FROM "${schema}".handler_probe WHERE queue='distributed-global' AND NOT finished`)
      ...
      await Promise.all([global.left.awaitResult(gLeft, wait), ...gRight.map((id) => global.right.awaitResult(id, wait))])
      const globalPeak = await admin.query<{ peak: number }>(`SELECT max(active_total)::integer AS peak FROM "${schema}".claim_audit WHERE queue='distributed-global'`)
      assert.equal(globalPeak.rows[0]?.peak, 2)
      console.log('PASS global concurrency=2 across two independently running Node worker processes')
      ...
      await Promise.all([keyed.right.awaitResult(blocked, wait), keyed.right.awaitResult(other, wait)])
      const keyPeak = await admin.query<{ peak: number }>(`SELECT max(active_key)::integer AS peak FROM "${schema}".claim_audit WHERE queue='distributed-keys'`)
      assert.equal(keyPeak.rows[0]?.peak, 1)
      console.log('PASS per-key serialization, unrelated-key progress and permit release after active cancellation')

      const rate = producer.get(RateQueue)
      const rLeft = await rate.left.enqueueMany(Array.from({ length: 4 }, (_, index) => ({ payload: { key: `left-${index}`, gate: 'rate' } })))
      const rRight = await rate.right.enqueueMany(Array.from({ length: 4 }, (_, index) => ({ payload: { key: `right-${index}`, gate: 'rate' } })))
      await Promise.all([...rLeft.map((id) => rate.left.awaitResult(id, wait)), ...rRight.map((id) => rate.right.awaitResult(id, wait))])
      const windows = await admin.query<{ window_start: string | null; total: number }>(`SELECT window_start,count(*)::integer AS total FROM "${schema}".claim_audit WHERE queue='distributed-rate' GROUP BY window_start ORDER BY window_start`)
      assert.ok(windows.rows.length >= 4)
      assert.ok(windows.rows.every((row) => row.window_start !== null && row.total <= 2), 'No persisted fixed window may admit more than two jobs across processes')
      assert.equal(windows.rows.reduce((sum, row) => sum + row.total, 0), 8)
      console.log('PASS shared fixed-window rate limit: all eight claims audited against persisted window identities')

      const permits = await admin.query<{ total: number }>(`SELECT count(*)::integer AS total FROM "${schema}".better_effect_mq_controlled_permits`)
      assert.equal(permits.rows[0]?.total, 0, 'Completion and cancellation must release every permit')
    } finally { await producer.close() }
    for (const child of children) await child.stop()
    children.length = 0
    const restarted = await NestFactory.createApplicationContext(ProducerModule, { logger: false, abortOnError: false })
    try { assert.equal((await restarted.get(MqQueueControlsService).get(GlobalQueue))?.revision, revision) }
    finally { await restarted.close() }
    console.log('PASS policy validation and revision persistence after all producer/worker contexts close')
  } finally {
    await admin.query(`UPDATE "${schema}".gates SET opened=true`).catch((cause) => { console.error('Test gate cleanup failed', cause) })
...
else console.log('Packed controls declarations/types passed; independent-process execution runs in PostgreSQL CI')
