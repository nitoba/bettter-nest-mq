...
export async function verifyOutboxSafety(connectionString: string): Promise<void> {
  ...
  try {
    ...
    class Producer {}
    const app = await NestFactory.createApplicationContext(Producer, {
      logger: false,
      abortOnError: false
    })
    let closed = false
    try {
      ...
      const prepare = async (id: string): Promise<OutboxEntry> => ({
        id,
        job: await queue.task.prepare({ value: id }, { jobId: id }),
        attempts: 3
      })
      ...
      assert.equal(
        (await admin.query(`SELECT 1 FROM "${schema}".business WHERE id='drained'`)).rowCount,
        1
      )
      ...
      await assert.rejects(
        outbox.transaction(async (tx) => {
          ...
          await assert.rejects(
            tx.append({ ...rejected, job: { ...rejected.job, connection: 'unregistered' } })
          )
        })
      )
      assert.equal(await service.get('source', rejected.id), undefined)
      assert.equal(
        (await admin.query(`SELECT 1 FROM "${schema}".business WHERE id=$1`, [rejected.id]))
          .rowCount,
        0
      )
      console.log(
        'PASS admitted operations drain before commit and caught append failures still roll back'
      )
      ...
      await until(
        async () =>
          (
            await admin.query(
              `SELECT 1 FROM pg_stat_activity WHERE pid=$1 AND query='SELECT pg_sleep(10)' AND state='active'`,
              [pid]
            )
          ).rowCount === 1,
        'transaction query starts'
      )
      await admin.query('SELECT pg_terminate_backend($1)', [pid])
      await failed
      assert.equal(executions, 1, 'A lost transaction must never rerun its business callback')
      assert.equal(
        (await admin.query(`SELECT 1 FROM "${schema}".business WHERE id=$1`, [lost.id])).rowCount,
        0
      )
      ...
      console.log(
        'PASS active transaction disconnection rolls back, releases the failed client and never replays the callback'
      )
      ...
      closed = true
      assert.equal(
        (await admin.query(`SELECT 1 FROM "${schema}".business WHERE id=$1`, [closingEntry.id]))
          .rowCount,
        1
      )
      assert.equal(
        (
          await admin.query(
            `SELECT 1 FROM "${schema}".better_effect_mq_outbox WHERE id=$1 AND state='pending'`,
            [closingEntry.id]
          )
        ).rowCount,
        1
      ...
      console.log(
        'PASS shutdown drains the already-admitted domain transaction before releasing its outbox resource'
      )
    } finally {
    ...
    @Module({
      imports: [
        MqModule.forRoot({
          ...
          outbox: { pollIntervalMs: 10, retryBaseDelayMs: 10, retryMaxDelayMs: 50 }
        })
      ]
    })
    class IncompletePublisher {}
    const publisher = await NestFactory.createApplicationContext(IncompletePublisher, {
      logger: false,
      abortOnError: false
    })
    try {
      ...
      await until(
        async () => (await service.counts('source')).failed === 2,
        'missing target routes become classified terminal failures'
      )
      ...
      assert.equal(
        (await admin.query(`SELECT 1 FROM "${schema}".better_effect_mq_jobs`)).rowCount,
        0
      )
      console.log(
        'PASS missing-route failures stay in the outbox without publishing to another destination'
      )
    } finally {
